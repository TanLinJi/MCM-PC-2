...
DEFAULT_EXP_ID="E1_36_modelnet_c_full_15_prompts_10_image_5_pointcloud_manual60_llm40"
...
if [[ "${USE_DEFAULT_PROMPTS}" == "1" ]]; then
  ...
  PROMPT_CACHE_DIR_VALUE="${E1_GENERATED_PROMPT_DIR:-llm/generated/E1_36_15_prompts_10_image_5_pointcloud_${GENERATED_TAG}}"
  PROMPT_CACHE_FILE_VALUE="${E1_GENERATED_PROMPT_FILE:-${DEFAULT_PROMPT_FILE}}"

  echo "============================================================"
  echo "E1_36 ModelNet-C full prompt generation mode"
...
bash "${SCRIPT_DIR}/common/run_modelnet_c_fusion.sh" \
  ...
  "${LLM_PROMPT_MODE_VALUE}" \
  "${PROMPT_CACHE_FILE_VALUE}" \
  "ModelNet-C full validation of selected E1_33 setting, 15 prompts = 10 image + 5 pointcloud, manual60_llm40" \
  "${PHYSICAL_GPU}"
