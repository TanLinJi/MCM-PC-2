...
DEFAULT_EXP_ID="E1_20_modelnet_c_full_15_prompts_10_image_5_pointcloud_manual75_llm25"
...
if [[ "${USE_DEFAULT_PROMPTS}" == "1" ]]; then
  ...
  PROMPT_CACHE_DIR_VALUE="${E1_GENERATED_PROMPT_DIR:-llm/generated/E1_20_15_prompts_10_image_5_pointcloud_${GENERATED_TAG}}"
  PROMPT_CACHE_FILE_VALUE="${E1_GENERATED_PROMPT_FILE:-${DEFAULT_PROMPT_FILE}}"

  echo "============================================================"
  echo "E1_20 prompt generation mode"
...
bash "${SCRIPT_DIR}/common/run_modelnet_c_fusion.sh" \
  ...
  "${LLM_PROMPT_MODE_VALUE}" \
  "${PROMPT_CACHE_FILE_VALUE}" \
  "ModelNet-C full prompt composition ablation, 15 prompts = 10 image + 5 pointcloud, manual75_llm25" \
  "${PHYSICAL_GPU}"
