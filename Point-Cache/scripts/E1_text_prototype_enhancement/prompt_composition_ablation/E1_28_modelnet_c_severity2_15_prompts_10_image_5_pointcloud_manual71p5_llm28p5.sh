...
DEFAULT_EXP_ID="E1_28_modelnet_c_severity2_15_prompts_10_image_5_pointcloud_manual71p5_llm28p5"
...
if [[ "${USE_DEFAULT_PROMPTS}" == "1" ]]; then
  ...
  PROMPT_CACHE_DIR_VALUE="${E1_GENERATED_PROMPT_DIR:-llm/generated/E1_28_15_prompts_10_image_5_pointcloud_${GENERATED_TAG}}"
  PROMPT_CACHE_FILE_VALUE="${E1_GENERATED_PROMPT_FILE:-${DEFAULT_PROMPT_FILE}}"

  echo "============================================================"
  echo "E1_28 severity2 prompt generation mode"
...
bash "${SCRIPT_DIR}/common/run_modelnet_c_fusion.sh" \
  ...
  "${LLM_PROMPT_MODE_VALUE}" \
  "${PROMPT_CACHE_FILE_VALUE}" \
  "ModelNet-C severity2 prompt composition diagnostic, 15 prompts = 10 image + 5 pointcloud, manual71.5_llm28.5" \
  "${PHYSICAL_GPU}"
