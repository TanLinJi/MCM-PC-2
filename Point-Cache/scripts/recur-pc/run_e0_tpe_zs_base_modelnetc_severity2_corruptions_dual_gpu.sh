...
EXP_NAME="e0_tpe_zs_base_modelnetc_severity2_corruptions"
...
run_one() {
  ...
  echo "[ZS-BASE] Start: ${corruption} on GPU ${gpu_id}"
  ...
  if [[ "${exit_code}" -ne 0 ]]; then
    ...
    echo "[ZS-BASE] Failed: ${corruption}. See ${log_file}"
    return 0
  fi

  acc="$(grep -E "Final|Zero-shot|accuracy|Acc" "${log_file}" | tail -n 5 | grep -oE '[0-9]+(\.[0-9]+)?' | tail -n 1 || true)"
  ...
  echo "[ZS-BASE] Finished: ${corruption} | GPU ${gpu_id} | Status ${status} | Accuracy ${acc}"
}
