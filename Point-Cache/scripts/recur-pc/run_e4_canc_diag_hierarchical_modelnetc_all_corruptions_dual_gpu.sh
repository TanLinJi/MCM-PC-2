...
# It only collects diagnostic statistics.
#
# GPU0:
...
RUN_DIR="${POINT_CACHE_ROOT}/logs/recur-pc/e4_canc_diag_dual_gpu_modelnetc_all_corruptions_${TIMESTAMP}"

SUMMARY_GPU0="${RUN_DIR}/summary_gpu0.csv"
SUMMARY_GPU1="${RUN_DIR}/summary_gpu1.csv"
SUMMARY_ALL="${RUN_DIR}/summary_e4_canc_diag_dual_gpu_all.csv"

RUNNER="${POINT_CACHE_ROOT}/runners/model_with_hierarchical_caches_e4_canc_diag.py"

CSV_HEADER="method,gpu,corruption,accuracy,global_error_rate,conflict_mean_rate,conflict_max_rate,wrong_given_conflict_mean,wrong_given_conflict_max,wrong_given_safe_mean,wrong_given_safe_max,local_alt_correct_given_conflict_mean,local_alt_correct_given_conflict_max,boundary_rate,uncertain_rate,candidate_or_mean_rate,candidate_safe_mean_rate,candidate_or_max_rate,candidate_safe_max_rate,status,log_file,json_file"
...
echo "============================================================"
echo "E4-CANC-DIAG Dual-GPU ModelNet-C Evaluation"
echo "============================================================"
echo "Time: ${TIMESTAMP}"
echo "Runner: ${RUNNER}"
...
fields = [
    ...
    "conflict_mean_rate",
    "conflict_max_rate",
    "wrong_given_conflict_mean",
    "wrong_given_conflict_max",
    "wrong_given_safe_mean",
    "wrong_given_safe_max",
    "local_alt_correct_given_conflict_mean",
    "local_alt_correct_given_conflict_max",
    "boundary_rate",
    "uncertain_rate",
    "candidate_or_mean_rate",
    "candidate_safe_mean_rate",
    "candidate_or_max_rate",
    "candidate_safe_max_rate",
    "status",
    "log_file",
    "json_file",
]
...
run_worker() {
  ...
  for COR_TYPE in "${cor_types[@]}"; do
    local LOG_FILE="${RUN_DIR}/e4_canc_diag_gpu${gpu_id}_${COR_TYPE}.log"
    local JSON_FILE="${RUN_DIR}/e4_canc_diag_gpu${gpu_id}_${COR_TYPE}.json"

    echo "============================================================" | tee "${LOG_FILE}"
    echo "[E4-CANC-DIAG] Start: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    ...
    echo "============================================================" | tee -a "${LOG_FILE}"
    echo "[E4-CANC-DIAG] Finished: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    echo "GPU_ID: ${gpu_id}" | tee -a "${LOG_FILE}"
