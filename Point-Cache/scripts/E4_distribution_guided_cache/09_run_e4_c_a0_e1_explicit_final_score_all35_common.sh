...
if [[ "$#" -lt 6 ]]; then
  echo "Usage: bash 09_run_e4_c_a0_e1_explicit_final_score_all35_common.sh SETTING ENTROPY_CAP GPA_CAP LOCAL_CAP NEG_CAP GPU"
  exit 1
fi
...
FINAL_SCORE_SWEEP_TAG="${E4_FINAL_SCORE_SWEEP_TAG:-ag4p4_al4p0_an0p18}"
EXP_ID="09_2_${SETTING}_${FINAL_SCORE_SWEEP_TAG}_ulip_modelnetc_all35_zs_global_local_e4_c_a0_e1_explicit_final_score_tw0p15_score_norm_manual_full_clip_manualfull_llm_dynamic_init_textdist"
LOCAL_CENTERS="${E4_LOCAL_CENTERS:-3}"
METHOD_FULL="09_2 all35 ${SETTING}: explicit final-score weight sweep; entropy_cap=${ENTROPY_CAP}; gpa_cap=${GPA_CAP}; local_cap=${LOCAL_CAP}; neg_cap=${NEG_CAP}; local_centers=${LOCAL_CENTERS}; formula=y_zs+alpha_g*y_g+alpha_l*y_l-alpha_n*y_n; text_weight=0.15; score_norm=running_zscore"
PURPOSE="基于 02_9_2/09_1 的新增 all35 代码；缓存更新和容量逻辑保持一致，只把 final score 写成显式公式 y=y_zs+alpha_g*y_g+alpha_l*y_l-alpha_n*y_n，并支持一次完整 ModelNet-C 推理统计多组 alpha。"
...
python "runners/E4_distribution_guided_cache/run_e4_c_a0_e1_explicit_final_score_ulip_modelnetc_all35.py" \
