...
if [[ "$#" -lt 4 ]]; then
  echo "Usage: bash 02_9_run_e4_c_a0_e1_textdist_only_text_weight_common.sh EXP_ID TEXT_WEIGHT PURPOSE GPU"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This E4-C-A0+E1-textdist text-weight experiment must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
bash "${SCRIPT_DIR}/02_run_e4_c_ulip_modelnetc_s2_common.sh" \
  ...
  "manual_full" \
  "E4-C-A0+E1-textdist-only text-weight ablation: text_weight=${TEXT_WEIGHT}" \
  "${PURPOSE}" \
  "${PHYSICAL_GPU}"
