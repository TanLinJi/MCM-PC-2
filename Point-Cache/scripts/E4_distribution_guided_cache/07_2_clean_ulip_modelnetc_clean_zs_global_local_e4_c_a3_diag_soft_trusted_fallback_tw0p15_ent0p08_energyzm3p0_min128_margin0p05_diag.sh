...
EXP_ID="07_2_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a3_diag_soft_trusted_fallback_tw0p15_ent0p08_energyzm3p0_min128_margin0p05_diag"
RUNNER="runners/E4_distribution_guided_cache/run_e4_c_a3_diag_soft_trusted_fallback_ulip_modelnetc_clean.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This A3 diagnostic clean run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
export E4_TRUSTED_ENERGY_Z_THRESHOLD="${E4_TRUSTED_ENERGY_Z_THRESHOLD:--3.0}"
...
export E4_SOFT_TRUSTED_MARGIN="${E4_SOFT_TRUSTED_MARGIN:-0.05}"
...
echo "============================================================"
echo "07_2 clean ModelNet-C A3 diagnostic soft trusted fallback run"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "07_2 clean: E4-C-A3 diagnostic soft trusted fallback tw0.15 entropy<=0.08 energy_z<=-3.0 min_count=128 margin=0.05 diag=sample_energy+replacement_snapshot on modelnet_c clean.h5" \
