...
EXP_ID="02_10_2_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a0_e1_distfinal_tw0p15_text1_visual1_score_norm_manualfull_llm_dynamic_init_textdist"
RUNNER="runners/E4_distribution_guided_cache/run_e4_c_a0_e1_distribution_final_score_ulip_modelnetc_clean.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This clean distribution-final run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "02_10_2 clean E4-C-A0+E1 distribution-final score"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "02_10_2 clean: E4-C-A0+E1 distribution-final score, text/visual final weights=1.0/1.0, no clip logits/cache voting final score" \
