...
EXP_ID="02_15_1_ulip_modelnetc_s2_e4_c_a0_e1_33_fused_prototype_text_gate_tw0p15_manual60_llm40"
...
echo "============================================================"
echo "02_15_1 ModelNet-C severity=2 fused-prototype text gate"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
if [[ "${E4_DRY_RUN}" == "1" ]]; then
  ...
  printf 'bash %q %q %q %q %q %q %q\n' \
    ...
    "${RUNNER}" \
    "manual_full" \
    "02-15-1: 02-9-2 carrier with E1-33 fused-prototype text gate; final Point-Cache voting unchanged; text_weight=0.15; manual_full=0.60, LLM=0.40" \
    "S2 diagnostic for fixing 02_14_1 negative transfer by using E1-33 fused prototype score in cache replacement gate instead of prompt-level Gaussian text score" \
    "${PHYSICAL_GPU}"
  exit 0
fi
