...
bash "${SCRIPT_DIR}/02_run_e4_c_ulip_modelnetc_s2_common.sh" \
  "02_2_ulip_modelnetc_s2_zs_global_local_e4_c_a0_score_norm_accepted_history_text_visual_distribution_guided_gpa_manual_full" \
  "runners/E4_distribution_guided_cache/run_e4_c_ulip_modelnetc_s2_accepted_history_text_visual_distribution_guided_gpa.py" \
  "manual_full" \
  "E4-C-A0: score-normalized accepted-history text-visual distribution-guided GPA-Cache" \
  "文本-视觉类别概率分布引导的 GPA-Cache 净化；在融合 text_score 与 visual_score 前使用 running z-score 做分数尺度归一化；视觉分布累计曾被 EntropyCache 或 GPACache 接受过的历史可信样本；未被正缓存接受的样本不参与分布；文本分布来自 prompt-level embeddings；沿用未满直接加入、满后替换最高熵样本、低熵门控和最终预测公式" \
  "${1:-0}"
