...
# 02_18_1:
# D-vMF-0 directional statistics diagnostics on the current best 02_9_2 carrier.

PROJECT_ROOT="/root/autodl-tmp/MCM-PC-2"
PC_ROOT="${PROJECT_ROOT}/Point-Cache"

EXP_ID="02_18_1_ulip_modelnetc_s2_d_vmf0_directional_stats_diag"
RUNNER="runners/E4_distribution_guided_cache/run_02_18_1_ulip_modelnetc_s2_d_vmf0_directional_stats_diag.py"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "02_18_1 reuses the existing 02_9_2 prompt cache and must not call the LLM API."
  exit 1
fi
...
export D_VMF_TEXT_PRIOR_WEIGHT="${D_VMF_TEXT_PRIOR_WEIGHT:-1.0}"
export D_VMF_EPS="${D_VMF_EPS:-1e-6}"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "02_18_1 ModelNet-C severity=2 D-vMF-0 directional diagnostics"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
echo "D-vMF ROLE: diagnostics only; no cache update or prediction change"
...
echo "D_VMF_TEXT_PRIOR_WEIGHT: ${D_VMF_TEXT_PRIOR_WEIGHT}"
echo "D_VMF_EPS: ${D_VMF_EPS}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  echo "cd ${PC_ROOT}"
  echo "bash scripts/E4_distribution_guided_cache/02_18_1_ulip_modelnetc_s2_d_vmf0_directional_stats_diag.sh ${PHYSICAL_GPU}"
  exit 0
fi
...
python "${RUNNER}" \
  ...
  --baseline-method-full "02-18-1: D-vMF-0 directional statistics diagnostics on 02_9_2; no prediction/cache change" \
