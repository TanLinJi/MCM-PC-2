...
EXP_ID="06_4_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a2_diag_dual_low_trusted_fallback_tw0p15_ent0p08_energyzm3p0_min128_diag"
RUNNER="runners/E4_distribution_guided_cache/run_e4_c_a2_diag_dual_low_trusted_fallback_ulip_modelnetc_clean.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This A2 diagnostic clean run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
if [[ -z "${E4_TRUSTED_ENERGY_Z_THRESHOLD:-}" ]]; then
  export E4_TRUSTED_ENERGY_Z_THRESHOLD="-3.0"
fi
...
echo "============================================================"
echo "06_4 clean ModelNet-C A2 diagnostic dual-low trusted fallback run"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "06_4 clean: E4-C-A2 diagnostic dual-low trusted fallback tw0.15 entropy<=0.08 energy_z<=-3.0 min_count=128 diag=sample_energy+replacement_snapshot on modelnet_c clean.h5" \
