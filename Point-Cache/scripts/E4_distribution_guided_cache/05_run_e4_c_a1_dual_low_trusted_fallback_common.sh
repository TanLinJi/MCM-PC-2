...
if [[ "$#" -lt 7 ]]; then
  echo "Usage: bash 05_run_e4_c_a1_dual_low_trusted_fallback_common.sh EXP_ID TEXT_WEIGHT ENTROPY_THRESHOLD ENERGY_Z_THRESHOLD ENERGY_MIN_COUNT PURPOSE GPU"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This A1 run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "E4-C-A1 dual-low trusted fallback common runner"
echo "EXP_ID: ${EXP_ID}"
...
bash "${SCRIPT_DIR}/02_run_e4_c_ulip_modelnetc_s2_common.sh" \
  "${EXP_ID}" \
  "runners/E4_distribution_guided_cache/run_e4_c_a1_dual_low_trusted_fallback_ulip_modelnetc_s2_accepted_history_text_visual_distribution_guided_gpa.py" \
  "manual_full" \
  "05_1: E4-C-A1 dual-low trusted fallback, text_weight=${E4_TEXT_SCORE_WEIGHT}, entropy<=${E4_TRUSTED_ENTROPY_THRESHOLD}, energy_z<=${E4_TRUSTED_ENERGY_Z_THRESHOLD}, energy_min_count=${E4_TRUSTED_ENERGY_MIN_COUNT}" \
  "${PURPOSE}" \
  "${PHYSICAL_GPU}"
