...
EXP_ID="02_9_2_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a0_e1_textdist_only_tw0p15_score_norm_manual_full_clip_manualfull_llm_dynamic_init_textdist"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This clean 02_9_2 run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "02_9_2 clean ModelNet-C run"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "02_9_2 clean: E4-C-A0+E1-textdist-only tw0.15 on modelnet_c clean.h5" \
