...
EXP_ID="05_2_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a1_dual_low_trusted_fallback_tw0p15_ent0p10_energyzm0p5_min64"
...
export E4_TRUSTED_ENTROPY_THRESHOLD="${E4_TRUSTED_ENTROPY_THRESHOLD:-0.10}"
if [[ -z "${E4_TRUSTED_ENERGY_Z_THRESHOLD:-}" ]]; then
  export E4_TRUSTED_ENERGY_Z_THRESHOLD="-0.5"
fi
export E4_TRUSTED_ENERGY_MIN_COUNT="${E4_TRUSTED_ENERGY_MIN_COUNT:-64}"
export E4_TRUSTED_ENERGY_EPS="${E4_TRUSTED_ENERGY_EPS:-1e-6}"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "05_2 clean ModelNet-C A1 dual-low trusted fallback run"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "05_2 clean: E4-C-A1 dual-low trusted fallback tw0.15 entropy<=0.10 energy_z<=-0.5 min_count=64 on modelnet_c clean.h5" \
