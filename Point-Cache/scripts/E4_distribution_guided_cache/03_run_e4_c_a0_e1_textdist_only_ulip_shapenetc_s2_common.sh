...
if [[ "$#" -lt 4 ]]; then
  echo "Usage: bash 03_run_e4_c_a0_e1_textdist_only_ulip_shapenetc_s2_common.sh EXP_ID TEXT_WEIGHT PURPOSE GPU"
  exit 1
fi
...
RUNNER="runners/E4_distribution_guided_cache/run_e4_c_a0_e1_textdist_only_ulip_shapenetc_s2_accepted_history_text_visual_distribution_guided_gpa.py"

SHARED_PROMPT_FILE="${PC_ROOT}/results/E1_text_prototype_enhancement/shared_prompts/snv2_c_deepseek_deepseek-v4-pro_multiview_2d3d_10_prompts.json"
SHAPENETC_CLASS_FILE="${PC_ROOT}/data/shapenet_c/shape_names.txt"

if [[ ! -f "${SHAPENETC_CLASS_FILE}" ]]; then
  echo "ERROR: ShapeNet-C class file not found:"
  echo "  ${SHAPENETC_CLASS_FILE}"
  exit 1
fi

python - "${SHARED_PROMPT_FILE}" "${SHAPENETC_CLASS_FILE}" <<'PY'
import json
import sys
...
if not cache_path.exists():
    print("ShapeNet-C E1 prompt cache not found; the runner will call the configured LLM API and save it:")
    print(f"  {cache_path}")
    raise SystemExit(0)
...
if missing or short:
    print("ShapeNet-C E1 prompt cache is incomplete; the runner will call the configured LLM API for missing/short classes.")
    if missing:
...
print(
    "Verified shared ShapeNet-C E1 prompt cache: "
...
echo "============================================================"
echo "E4-C-A0+E1-textdist-only ShapeNet-C severity=2"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
echo "TEXT_WEIGHT: ${TEXT_WEIGHT}"
echo "PURPOSE: ${PURPOSE}"
echo "PHYSICAL_GPU: ${PHYSICAL_GPU}"
echo "Dataset loader: snv2_c"
echo "Data root: data/shapenet_c"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E4-C-A0+E1-textdist-only text-weight ablation on ShapeNet-C: text_weight=${TEXT_WEIGHT}" \
