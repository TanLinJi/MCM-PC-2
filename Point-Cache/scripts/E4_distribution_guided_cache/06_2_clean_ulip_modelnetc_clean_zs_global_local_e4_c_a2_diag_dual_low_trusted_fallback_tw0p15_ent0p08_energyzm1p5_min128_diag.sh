...
EXP_ID="06_2_clean_ulip_modelnetc_clean_zs_global_local_e4_c_a2_diag_dual_low_trusted_fallback_tw0p15_ent0p08_energyzm1p5_min128_diag"
...
if [[ -z "${E4_TRUSTED_ENERGY_Z_THRESHOLD:-}" ]]; then
  export E4_TRUSTED_ENERGY_Z_THRESHOLD="-1.5"
fi
...
echo "============================================================"
echo "06_2 clean ModelNet-C A2 diagnostic dual-low trusted fallback run"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "06_2 clean: E4-C-A2 diagnostic dual-low trusted fallback tw0.15 entropy<=0.08 energy_z<=-1.5 min_count=128 diag=sample_energy+replacement_snapshot on modelnet_c clean.h5" \
