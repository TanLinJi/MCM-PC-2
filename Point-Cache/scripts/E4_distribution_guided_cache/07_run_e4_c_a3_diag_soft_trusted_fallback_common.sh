...
if [[ "$#" -lt 8 ]]; then
  echo "Usage: bash 07_run_e4_c_a3_diag_soft_trusted_fallback_common.sh EXP_ID TEXT_WEIGHT ENTROPY_THRESHOLD ENERGY_Z_THRESHOLD ENERGY_MIN_COUNT SOFT_MARGIN PURPOSE GPU"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "This A3 diagnostic run must reuse the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
export E4_SOFT_TRUSTED_MARGIN="${E4_SOFT_TRUSTED_MARGIN:-${SOFT_MARGIN}}"
...
echo "============================================================"
echo "E4-C-A3 diagnostic soft trusted fallback common runner"
echo "EXP_ID: ${EXP_ID}"
...
echo "E4_SOFT_TRUSTED_MARGIN: ${E4_SOFT_TRUSTED_MARGIN}"
...
bash "${SCRIPT_DIR}/02_run_e4_c_ulip_modelnetc_s2_common.sh" \
  "${EXP_ID}" \
  "runners/E4_distribution_guided_cache/run_e4_c_a3_diag_soft_trusted_fallback_ulip_modelnetc_s2_accepted_history_text_visual_distribution_guided_gpa.py" \
  "manual_full" \
  "07_1: E4-C-A3 diagnostic soft trusted fallback, text_weight=${E4_TEXT_SCORE_WEIGHT}, entropy<=${E4_TRUSTED_ENTROPY_THRESHOLD}, energy_z<=${E4_TRUSTED_ENERGY_Z_THRESHOLD}, energy_min_count=${E4_TRUSTED_ENERGY_MIN_COUNT}, margin=${E4_SOFT_TRUSTED_MARGIN}, diag=sample_energy+replacement_snapshot" \
  "${PURPOSE}" \
  "${PHYSICAL_GPU}"
