...
if [[ "$#" -lt 4 ]]; then
  echo "Usage: bash 00_run_e5_a0_a1_ulip_modelnetc_s2_common.sh EXP_ID PURPOSE GPU RUNNER"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E5-A0/A1 reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
# E5-A0/A1 diagnostics.
export E5_STATSBANK_CAPACITY="${E5_STATSBANK_CAPACITY:-16}"
export E5_GDA_ALPHA="${E5_GDA_ALPHA:-0.9}"
export E5_GDA_MIN_TOTAL="${E5_GDA_MIN_TOTAL:-8}"
export E5_GDA_MIN_CLASSES="${E5_GDA_MIN_CLASSES:-2}"
export E5_GDA_NORM_EPS="${E5_GDA_NORM_EPS:-1e-6}"
export E5_GDA_NORM_CLIP="${E5_GDA_NORM_CLIP:-3.0}"
export E5_SAVE_SAMPLE_DIAGNOSTICS="${E5_SAVE_SAMPLE_DIAGNOSTICS:-1}"
export E5_GDA_OVERRIDE_THRESHOLDS="${E5_GDA_OVERRIDE_THRESHOLDS:-0.5,0.75,1.0,1.25,1.5,2.0}"
export E5_CORRUPTIONS="${E5_CORRUPTIONS:-}"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "E5-A0/A1 ADAPT-inspired StatsBank + shared-covariance GDA diagnostics"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
echo "E5_GDA_ALPHA: ${E5_GDA_ALPHA}"
echo "E5_GDA_MIN_TOTAL: ${E5_GDA_MIN_TOTAL}"
echo "E5_GDA_MIN_CLASSES: ${E5_GDA_MIN_CLASSES}"
echo "E5_GDA_NORM_EPS: ${E5_GDA_NORM_EPS}"
echo "E5_GDA_NORM_CLIP: ${E5_GDA_NORM_CLIP}"
echo "E5_SAVE_SAMPLE_DIAGNOSTICS: ${E5_SAVE_SAMPLE_DIAGNOSTICS}"
echo "E5_GDA_OVERRIDE_THRESHOLDS: ${E5_GDA_OVERRIDE_THRESHOLDS}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E5-A0/A1: E4-C-A0+E1-textdist-only baseline with delayed StatsBank shared-covariance GDA diagnostics" \
