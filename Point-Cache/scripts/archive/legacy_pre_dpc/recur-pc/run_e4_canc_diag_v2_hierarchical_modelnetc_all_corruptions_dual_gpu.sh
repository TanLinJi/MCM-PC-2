...
#
# This script does NOT change prediction behavior.
# It only collects diagnostic statistics with threshold sweep.
#
# GPU0:
...
RUN_DIR="${POINT_CACHE_ROOT}/logs/recur-pc/e4_canc_diag_v2_dual_gpu_modelnetc_all_corruptions_${TIMESTAMP}"

SUMMARY_GPU0="${RUN_DIR}/summary_gpu0.csv"
SUMMARY_GPU1="${RUN_DIR}/summary_gpu1.csv"
SUMMARY_ALL="${RUN_DIR}/summary_e4_canc_diag_v2_dual_gpu_all.csv"

RUNNER="${POINT_CACHE_ROOT}/runners/model_with_hierarchical_caches_e4_canc_diag_v2.py"

CSV_HEADER="method,gpu,corruption,accuracy,global_error_rate,d_mean_p90,d_mean_p95,d_mean_p99,d_mean_p999,d_max_p90,d_max_p95,d_max_p99,d_max_p999,best_mean_tau,best_mean_conflict_rate,best_mean_wrong_given_conflict,best_mean_local_alt_correct,best_max_tau,best_max_conflict_rate,best_max_wrong_given_conflict,best_max_local_alt_correct,best_safe_mean_tau,best_safe_mean_rate,best_safe_mean_wrong,best_safe_max_tau,best_safe_max_rate,best_safe_max_wrong,boundary_rate,uncertain_rate,status,log_file,json_file"
...
echo "============================================================"
echo "E4-CANC-DIAG-v2 Dual-GPU ModelNet-C Evaluation"
echo "============================================================"
echo "Time: ${TIMESTAMP}"
echo "Runner: ${RUNNER}"
...
fields = [
    ...
    "best_mean_tau",
    "best_mean_conflict_rate",
    "best_mean_wrong_given_conflict",
    "best_mean_local_alt_correct",

    "best_max_tau",
    "best_max_conflict_rate",
    "best_max_wrong_given_conflict",
    "best_max_local_alt_correct",
...
run_worker() {
  ...
  for COR_TYPE in "${cor_types[@]}"; do
    local LOG_FILE="${RUN_DIR}/e4_canc_diag_v2_gpu${gpu_id}_${COR_TYPE}.log"
    local JSON_FILE="${RUN_DIR}/e4_canc_diag_v2_gpu${gpu_id}_${COR_TYPE}.json"

    echo "============================================================" | tee "${LOG_FILE}"
    echo "[E4-CANC-DIAG-v2] Start: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    ...
    set +e
    E4_DIAG_OUT="${JSON_FILE}" CUDA_VISIBLE_DEVICES="${gpu_id}" python "${RUNNER}" \
      --config configs \
      --lm3d ulip \
    ...
    echo "============================================================" | tee -a "${LOG_FILE}"
    echo "[E4-CANC-DIAG-v2] Finished: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    echo "GPU_ID: ${gpu_id}" | tee -a "${LOG_FILE}"
