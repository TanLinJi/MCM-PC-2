...
EXP_NAME="e0_tpe_v2_meanlogit_zs_modelnetc_severity2_corruptions"
...
run_one() {
  ...
  echo "[E0-TPE-v2-meanlogit-ZS] Start: ${corruption} on GPU ${gpu_id}"

  set +e
  TPE_AGG_MODE="meanlogit" \
  ...
  if [[ "${exit_code}" -ne 0 ]]; then
    ...
    echo "[E0-TPE-v2-meanlogit-ZS] Failed: ${corruption}. See ${log_file}"
    return 0
  fi
  ...
  echo "[E0-TPE-v2-meanlogit-ZS] Finished: ${corruption} | GPU ${gpu_id} | Status ${status} | Accuracy ${acc}"
}
...
echo
echo "[E0-TPE-v2-meanlogit-ZS] All done."
