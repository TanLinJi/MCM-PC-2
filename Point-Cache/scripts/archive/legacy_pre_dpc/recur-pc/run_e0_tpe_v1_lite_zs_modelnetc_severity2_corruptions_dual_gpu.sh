...
EXP_NAME="e0_tpe_v1_lite_zs_modelnetc_severity2_corruptions"
...
run_one() {
  ...
  echo "[E0-TPE-v1-lite-ZS] Start: ${corruption} on GPU ${gpu_id}"

  set +e
  CUDA_VISIBLE_DEVICES="${gpu_id}" python runners/zs_infer_e0_tpe_v1_lite.py \
  ...
  if [[ "${exit_code}" -ne 0 ]]; then
    ...
    echo "[E0-TPE-v1-lite-ZS] Failed: ${corruption}. See ${log_file}"
    return 0
  fi

  acc="$(grep -E "Final.*Zero-shot test accuracy|Zero-shot test accuracy|accuracy" "${log_file}" \
    | tail -n 5 \
    | grep -oE '[0-9]+(\.[0-9]+)?' \
    | tail -n 1 || true)"
  ...
  echo "[E0-TPE-v1-lite-ZS] Finished: ${corruption} | GPU ${gpu_id} | Status ${status} | Accuracy ${acc}"
}
...
echo
echo "[E0-TPE-v1-lite-ZS] All done."
