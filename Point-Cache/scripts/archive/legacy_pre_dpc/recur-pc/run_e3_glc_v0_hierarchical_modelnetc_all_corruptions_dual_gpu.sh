...
# ============================================================
# E3-GLC: Dual-GPU ModelNet-C all-corruption evaluation
#
# GPU0 runs:
...
# We do NOT split one corruption across GPUs.
# ============================================================
...
RUN_DIR="${POINT_CACHE_ROOT}/logs/recur-pc/e3_glc_dual_gpu_modelnetc_all_corruptions_${TIMESTAMP}"
SUMMARY_GPU0="${RUN_DIR}/summary_gpu0.csv"
SUMMARY_GPU1="${RUN_DIR}/summary_gpu1.csv"
SUMMARY_ALL="${RUN_DIR}/summary_e3_glc_dual_gpu_all.csv"
...
echo "============================================================"
echo "E3-GLC Dual-GPU ModelNet-C all-corruption evaluation"
echo "============================================================"
echo "Time: ${TIMESTAMP}"
echo "Runner: ${RUNNER}"
echo "Run dir: ${RUN_DIR}"
echo "GPU0 summary: ${SUMMARY_GPU0}"
echo "GPU1 summary: ${SUMMARY_GPU1}"
...
run_worker() {
  ...
  for COR_TYPE in "${cor_types[@]}"; do
    local LOG_FILE="${RUN_DIR}/e3_glc_gpu${gpu_id}_${COR_TYPE}.log"

    echo "============================================================" | tee "${LOG_FILE}"
    echo "[E3-GLC] Start: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    ...
    echo "e3_glc_hierarchical,${gpu_id},${COR_TYPE},${ACC},${STATUS},${LOG_FILE}" >> "${summary_file}"

    echo "============================================================" | tee -a "${LOG_FILE}"
    echo "[E3-GLC] Finished: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    echo "GPU_ID: ${gpu_id}" | tee -a "${LOG_FILE}"
...
echo "[Launch workers]"
echo "============================================================"
echo "GPU0 corruptions: ${GPU0_COR_TYPES[*]}"
echo "GPU1 corruptions: ${GPU1_COR_TYPES[*]}"
