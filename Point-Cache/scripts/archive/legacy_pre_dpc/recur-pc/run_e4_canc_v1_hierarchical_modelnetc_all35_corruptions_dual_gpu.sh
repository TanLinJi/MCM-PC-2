...
RUN_DIR="${POINT_CACHE_ROOT}/logs/recur-pc/e4_canc_v1_dual_gpu_modelnetc_all35_corruptions_${TIMESTAMP}"

SUMMARY_GPU0="${RUN_DIR}/summary_gpu0.csv"
SUMMARY_GPU1="${RUN_DIR}/summary_gpu1.csv"
SUMMARY_ALL="${RUN_DIR}/summary_e4_canc_v1_dual_gpu_all35.csv"

RUNNER="${POINT_CACHE_ROOT}/runners/model_with_hierarchical_caches_e4_canc_v1.py"

CSV_HEADER="method,gpu,corruption,accuracy,base_negative_rate,e4_total_negative_rate,e4_extra_negative_rate,boundary_rate,conflict_rate,conflict_confident_rate,status,log_file,json_file"

if [[ "${QUICK_TEST:-0}" == "1" ]]; then
  GPU0_COR_TYPES=(add_global_2)
  GPU1_COR_TYPES=()
else
  # 35 corruptions = 7 corruption types × 5 severity levels.
  #
  # GPU0: 18 corruptions
  GPU0_COR_TYPES=(
    add_global_0
...
echo "============================================================"
echo "E4-CANC-v1 Dual-GPU ModelNet-C Evaluation"
echo "============================================================"
echo "Time: ${TIMESTAMP}"
echo "Runner: ${RUNNER}"
...
run_worker() {
  ...
  for COR_TYPE in "${cor_types[@]}"; do
    local LOG_FILE="${RUN_DIR}/e4_canc_v1_gpu${gpu_id}_${COR_TYPE}.log"
    local JSON_FILE="${RUN_DIR}/e4_canc_v1_gpu${gpu_id}_${COR_TYPE}.json"

    echo "============================================================" | tee "${LOG_FILE}"
    echo "[E4-CANC-v1] Start: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    ...
    echo "============================================================" | tee -a "${LOG_FILE}"
    echo "[E4-CANC-v1] Finished: ${COR_TYPE}" | tee -a "${LOG_FILE}"
    echo "GPU_ID: ${gpu_id}" | tee -a "${LOG_FILE}"
