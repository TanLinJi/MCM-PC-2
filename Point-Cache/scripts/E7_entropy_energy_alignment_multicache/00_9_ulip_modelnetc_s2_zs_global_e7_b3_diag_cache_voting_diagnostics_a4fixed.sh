...
# E7-B3-Diag corrupted entry:
# ULIP + ModelNet-C severity=2, cache-voting diagnostics on A4 fixed.
...
EXP_ID="00_9_ulip_modelnetc_s2_zs_global_e7_b3_diag_cache_voting_diagnostics_a4fixed"
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_b3_diag_ulip_modelnetc_s2_cache_voting_diagnostics.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-B3-Diag reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "E7-B3-Diag Cache-Voting Diagnostics on ModelNet-C severity=2"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-B3-Diag: A4 fixed cache-voting diagnostics only, no prediction change" \
