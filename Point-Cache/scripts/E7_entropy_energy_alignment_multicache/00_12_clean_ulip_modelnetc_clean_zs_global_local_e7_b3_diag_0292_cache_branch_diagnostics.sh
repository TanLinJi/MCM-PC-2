...
# E7-B3-Diag-02_9_2 clean entry:
# ULIP + ModelNet-C clean.h5, cache-branch diagnostics on the current best 02_9_2 carrier.
...
EXP_ID="00_12_clean_ulip_modelnetc_clean_zs_global_local_e7_b3_diag_0292_cache_branch_diagnostics"
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_b3_diag_0292_ulip_modelnetc_clean_cache_branch_diagnostics.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-B3-Diag-02_9_2 clean reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "E7-B3-Diag-02_9_2 Cache-Branch Diagnostics on ModelNet-C clean"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-B3-Diag-02_9_2 clean: cache-branch diagnostics only, no prediction change" \
