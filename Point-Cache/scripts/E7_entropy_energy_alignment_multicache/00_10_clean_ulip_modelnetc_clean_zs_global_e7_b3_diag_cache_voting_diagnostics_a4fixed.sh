...
# E7-B3-Diag clean entry:
# ULIP + ModelNet-C clean.h5, cache-voting diagnostics on A4 fixed.
...
EXP_ID="00_10_clean_ulip_modelnetc_clean_zs_global_e7_b3_diag_cache_voting_diagnostics_a4fixed"
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_b3_diag_ulip_modelnetc_clean_cache_voting_diagnostics.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-B3-Diag clean run reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
# Replacement distribution settings inherited from A4/02_9_2.
...
echo "============================================================"
echo "E7-B3-Diag Cache-Voting Diagnostics on ModelNet-C clean"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-B3-Diag clean: A4 fixed cache-voting diagnostics only, no prediction change" \
