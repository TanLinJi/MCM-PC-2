...
# E7-A common launcher: ULIP + ModelNet-C severity=2 entropy/energy/alignment multi-cache.
#
# 用法:
#   bash 00_run_e7_a_ulip_modelnetc_s2_common.sh EXP_ID PURPOSE GPU
#
# 该脚本:
#   1. 校验 E1 共享 prompt 缓存完整，不调用 LLM API；
#   2. 导出 E7_* 超参数（可被外部环境变量覆盖）；
#   3. 路由到 E7-A runner，cache_type 在 runner 内部强制为 global。

if [[ "$#" -lt 3 ]]; then
  echo "Usage: bash 00_run_e7_a_ulip_modelnetc_s2_common.sh EXP_ID PURPOSE GPU"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-A reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
# 缓存容量（设计文档 7）。
export E7_ENTROPY_CAPACITY="${E7_ENTROPY_CAPACITY:-5}"
export E7_ENERGY_CAPACITY="${E7_ENERGY_CAPACITY:-5}"
export E7_ALIGNMENT_CAPACITY="${E7_ALIGNMENT_CAPACITY:-3}"
...
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_a_ulip_modelnetc_s2_entropy_energy_alignment_cache.py"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "E7-A Entropy-Energy-Alignment Multi-Cache (global feature only)"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-A entropy/energy/alignment multi-cache (manual_full classifier, E1 textdist replacement only)" \
