...
# E7-A4-B1 common launcher: ULIP + ModelNet-C severity=2 cache-norm-clipped candidate-pool alignment-core cache.
#
# 用法:
#   bash 00_run_e7_a4_b1_ulip_modelnetc_s2_common.sh EXP_ID PURPOSE GPU
#
# 该脚本:
#   1. 校验 E1 共享 prompt 缓存完整，不调用 LLM API；
#   2. 导出 E7-A4-B1 超参数（可被外部环境变量覆盖）；
#   3. 路由到 E7-A4-B1 runner，cache_type 在 runner 内部强制为 global。

if [[ "$#" -lt 3 ]]; then
  echo "Usage: bash 00_run_e7_a4_b1_ulip_modelnetc_s2_common.sh EXP_ID PURPOSE GPU"
  exit 1
fi
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-A4-B1 reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
export E7_CACHE_NORM_CAP="${E7_CACHE_NORM_CAP:-20.0}"
export E7_CACHE_NORM_EPS="${E7_CACHE_NORM_EPS:-1e-6}"
...
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_a4_b1_ulip_modelnetc_s2_cache_norm_clip.py"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "E7-A4-B1 Cache-Norm-Clipped Candidate-Pool Alignment-Core Cache (global feature only)"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
echo "E7_CACHE_NORM_CAP: ${E7_CACHE_NORM_CAP}"
echo "E7_CACHE_NORM_EPS: ${E7_CACHE_NORM_EPS}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-A4-B1 cache-norm-clipped candidate-pool alignment-core cache cap=20 (manual_full classifier, E1 textdist replacement only)" \
