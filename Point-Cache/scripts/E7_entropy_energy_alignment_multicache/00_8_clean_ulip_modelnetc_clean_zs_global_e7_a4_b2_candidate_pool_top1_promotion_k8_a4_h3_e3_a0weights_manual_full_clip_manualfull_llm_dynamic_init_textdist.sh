...
# E7-A4-B2 clean entry:
# ULIP + ModelNet-C clean.h5, candidate-pool top1 promotion.
...
EXP_ID="00_8_clean_ulip_modelnetc_clean_zs_global_e7_a4_b2_candidate_pool_top1_promotion_k8_a4_h3_e3_a0weights_manual_full_clip_manualfull_llm_dynamic_init_textdist"
RUNNER="runners/E7_entropy_energy_alignment_multicache/run_e7_a4_b2_ulip_modelnetc_clean_candidate_pool_top1_promotion.py"
PHYSICAL_GPU="${1:-0}"
...
if [[ ! -f "${SHARED_PROMPT_FILE}" ]]; then
  ...
  echo
  echo "E7-A4-B2 clean run reuses the existing prompt cache and must not call the LLM API."
  exit 1
fi
...
echo "============================================================"
echo "E7-A4-B2 Candidate-Pool Top1 Promotion on ModelNet-C clean"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
python "${RUNNER}" \
  ...
  --baseline-method-full "E7-A4-B2 clean: candidate-pool top1 promotion, Kcand=8 Kalign=4 KH=3 KE=3, A0 weights, manual_full classifier, E1 textdist replacement only" \
