...
if [[ "$#" -lt 5 ]]; then
  echo "Usage: bash 01_run_ulip_modelnetc_s2_textproto_guard_common.sh EXP_ID RUNNER PROMPT_SOURCE METHOD_FULL PURPOSE [GPU]"
  exit 1
fi
...
export TEXT_PROTO_GUARD_RHO="${TEXT_PROTO_GUARD_RHO:-0.05}"

mkdir -p "${WANDB_DIR}"

echo "============================================================"
echo "E3-V2-TextProto-Guard-C"
echo "EXP_ID: ${EXP_ID}"
echo "RUNNER: ${RUNNER}"
...
echo "TEXT_PROTO_GUARD_RHO: ${TEXT_PROTO_GUARD_RHO}"
echo "============================================================"
