# 本公共脚本用于执行 OpenShape 在 ScanObjNN clean hardest 上的单方法 baseline 复现实验。
#!/usr/bin/env bash
set -euo pipefail

if [[ "$#" -lt 5 ]]; then
  echo "Usage: bash 23_run_openshape_scanobjnn_clean_hardest_common.sh EXP_ID METHOD METHOD_FULL RUNNER CACHE_TYPE [GPU]"
  exit 1
fi
...
echo "OpenShape version: ${OSHAPE_VERSION}"
echo "Runner: ${RUNNER}"
...
if [[ ! -f "${CKPT_PATH}" ]]; then
  ...
  echo "ERROR: missing OpenShape checkpoint ${CKPT_PATH}"
  exit 1
fi

if [[ ! -f "${OPENCLIP_PATH}" ]]; then
  echo "${EXP_ID},${DATASET},${DATA_ROOT},${CORRUPTION},${SEVERITY},${COR_TYPE},${DATA_FILE},${SONN_VARIANT},${BACKBONE},${METHOD},${METHOD_FULL},,missing_openclip_ckpt,${PHYSICAL_GPU},${LOG_FILE}" >> "${SUMMARY_FILE}"
  echo "ERROR: missing OpenCLIP checkpoint ${OPENCLIP_PATH}"
  exit 1
fi
