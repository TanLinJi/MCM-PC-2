# 本公共脚本用于执行 ULIP 在 ScanObjNN clean hardest 上的单方法 baseline 复现实验。
#!/usr/bin/env bash
set -euo pipefail

if [[ "$#" -lt 5 ]]; then
  echo "Usage: bash 03_run_ulip_scanobjnn_clean_hardest_common.sh EXP_ID METHOD METHOD_FULL RUNNER CACHE_TYPE [GPU]"
  exit 1
fi
