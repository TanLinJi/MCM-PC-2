# 本公共脚本用于执行 Uni3D 在 ScanObjNN-C hardest 全部 35 个损坏设置上的单方法复现实验；当前版本只启动一次 Python，并在 Python 内部循环 35 个 cor_type。
#!/usr/bin/env bash
set -euo pipefail

if [[ "$#" -lt 5 ]]; then
  echo "Usage: bash 34_run_uni3d_scanobjnnc_hardest_corruptions_all35_common.sh EXP_ID METHOD METHOD_FULL RUNNER CACHE_TYPE [GPU]"
  exit 1
fi
...
echo "Python runner: runners/baseline/run_uni3d_scanobjnnc_hardest_corruptions_all35.py"
echo "============================================================"

python runners/baseline/run_uni3d_scanobjnnc_hardest_corruptions_all35.py \
