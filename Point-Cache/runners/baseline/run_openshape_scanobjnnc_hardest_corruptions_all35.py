#!/usr/bin/env python
# 本 runner 用于在单个 Python 进程中复现 OpenShape × ScanObjNN-C hardest 全 35 个损坏设置的一种 baseline 方法，避免 35 次重复加载模型。
import argparse
import csv
import gc
import io
    ...
    if clip_weights_state["clip_weights"] is None:
        ...
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                ...
                "clip_weights must be rebuilt, but this runner expects fixed SONN-C classes."
            )
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"data_file={Path(data_root) / sonn_variant / (cor_type + '.h5')}")
...
def main():
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("Optimized OpenShape ScanObjNN-C hardest all35 baseline runner")
        print(f"EXP_ID: {exp_id}")
        print(f"Method: {method_full}")
        print(f"Physical GPU: {physical_gpu}")
        print("Internal device: 0")
        print(f"Dataset: {args.dataset}")
        print(f"SONN variant: {sonn_variant}")
    ...
    for corruption in CORRUPTIONS:
        for severity in SEVERITIES:
            cor_type = f"{corruption}_{severity}"
            data_file = Path(data_root) / sonn_variant / f"{cor_type}.h5"
