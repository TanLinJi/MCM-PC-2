...
E4-CANC-v0:
    I_neg = I_H OR (I_M AND I_D)

where:
    I_H: original entropy-based negative candidate
    I_M: global boundary candidate
...
# ============================================================
# Main E4-CANC-v0 loop
# ============================================================

@torch.no_grad()
def run_test_tda_e4_canc_v0(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print(f"[E4-CANC-v0] TAU_D={TAU_D}, TAU_M={TAU_M}, TAU_P={TAU_P}")
    print(f"[E4-CANC-v0] Boundary feasibility: {feasible_boundary}")

    if not feasible_boundary:
        print("[E4-CANC-v0][WARNING] Boundary candidate may be empty under current thresholds.")
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        I_E4_extra = I_M and I_D
        I_neg_e4 = I_H or I_E4_extra

        n += 1
        base_neg_count += int(I_H)
        boundary_count += int(I_M)
        conflict_count += int(I_D)
        conflict_boundary_count += int(I_E4_extra)
        ...
        # ---------------- negative cache: E4-CANC-v0 change ----------------
    ...
    print("E4_CANC_V0_SUMMARY " + json.dumps(summary, sort_keys=True))

    out_path = os.environ.get("E4_CANC_OUT", "")
    if out_path:
        with open(out_path, "w") as f:
            json.dump(summary, f, indent=2, sort_keys=True)
        print(f"[E4-CANC-v0] Summary JSON saved to: {out_path}")
