#!/usr/bin/env python
"""
E7-A runner: ULIP + ModelNet-C severity=2, entropy/energy/alignment multi-cache.
...
2. 路由到 E7-A 的多缓存模型 run_test_tda；
...
from runners.E7_entropy_energy_alignment_multicache.model_e7_a_entropy_energy_alignment_cache import (
    run_test_tda as run_e7_multicache,
)
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"[E7-A] clip (final classifier) prompt source: {args.prompt_source}")
                print(f"[E7-A] text distribution prompt source: {args.e7_text_dist_prompt_source}")
                print(f"[E7-A] text_dist classes: {len(text_dist)}")
...
def main():
    ...
    args.e7_variant = "E7-A-entropy-energy-alignment-multi-cache"
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E7-A ULIP ModelNet-C severity=2 Entropy-Energy-Alignment Multi-Cache runner")
        ...
        print("Variant: E7-A entropy/energy/alignment multi-cache (global feature only, no local cache)")
