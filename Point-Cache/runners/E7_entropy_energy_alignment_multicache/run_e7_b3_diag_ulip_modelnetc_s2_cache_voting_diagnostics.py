#!/usr/bin/env python
"""
E7-B3-Diag runner: ULIP + ModelNet-C severity=2, cache-voting diagnostics.
...
2. 路由到 E7-B3-Diag 的 A4-fixed 缓存投票诊断模型 run_test_tda；
...
from runners.E7_entropy_energy_alignment_multicache.model_e7_b3_diag_cache_voting_diagnostics import (
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"[E7-B3-Diag] clip (final classifier) prompt source: {args.prompt_source}")
                print(f"[E7-B3-Diag] text distribution prompt source: {args.e7_text_dist_prompt_source}")
                print(f"[E7-B3-Diag] text_dist classes: {len(text_dist)}")
...
def main():
    ...
    args.e7_variant = "E7-B3-Diag-cache-voting-diagnostics-a4fixed"
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E7-B3-Diag ULIP ModelNet-C severity=2 Cache-Voting Diagnostics runner")
        ...
        print("Variant: E7-B3-Diag cache-voting diagnostics on A4 fixed (global feature only, no local cache)")
