"""
E7-B3-Diag-02_9_2: Cache-Branch Diagnostics on the current best 02_9_2 carrier.

核心思想：
1. 载体完全沿用 02_9_2：E4-C-A0+E1-textdist-only, text_weight=0.15；
2. 不改变缓存更新规则；
3. 不改变正式最终预测公式；
4. 只额外记录零样本文本原型点积得分、全局熵缓存、GPA 全局诊断得分、
   GPA 局部缓存、负缓存和最终得分的逐分支正确数/总数。
"""
...
GPA_VARIANT_NAME = "E7-B3-Diag-02_9_2-cache-branch-diagnostics"
...
def _save_b3_diag_0292_stats(
    ...
    out_dir = Path(result_root) / exp_id / "e7_b3_diag_0292_stats"
    ...
    payload = {
        ...
        "e4_variant": "02_9_2 carrier: E4-C-A0+E1-textdist-only",
        "b3_diag": {
            "carrier": "02_9_2",
    ...
    filename = f"{getattr(args, 'cor_type', 'unknown')}_e7_b3_diag_0292_stats.json"
    ...
    if event_records is not None:
        ...
        print(f"[E7-B3-Diag-02_9_2] Saved GPA replacement events to {event_path}")

    if B3_DIAG_SAVE_SAMPLES and diag_samples:
        sample_path = out_dir / f"{getattr(args, 'cor_type', 'unknown')}_e7_b3_diag_0292_samples.jsonl"
        with sample_path.open("w", encoding="utf-8") as f:
            for item in diag_samples:
                f.write(json.dumps(item, ensure_ascii=False) + "\n")
        print(f"[E7-B3-Diag-02_9_2] Saved sample diagnostics to {sample_path}")

    print(f"[E7-B3-Diag-02_9_2] Saved stats to {out_dir / filename}")
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    print("[E7-B3-Diag-02_9_2] carrier: 02_9_2 / E4-C-A0+E1-textdist-only")
    print("[E7-B3-Diag-02_9_2] len(entropy_cache):", len(entropy_cache))
    print("[E7-B3-Diag-02_9_2] len(gpa_cache):", len(gpa_cache))
    print("[E7-B3-Diag-02_9_2] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E7-B3-Diag-02_9_2] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E7-B3-Diag-02_9_2] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E7-B3-Diag-02_9_2] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E7-B3-Diag-02_9_2] visual distribution classes:", len(visual_dist))
    print("[E7-B3-Diag-02_9_2] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E7-B3-Diag-02_9_2] score norm mode:", SCORE_NORM_MODE)
    print("[E7-B3-Diag-02_9_2] score norm state:", _summarize_score_norm_state(score_norm_state))
    print("[E7-B3-Diag-02_9_2] save sample diagnostics:", B3_DIAG_SAVE_SAMPLES)
    print("[E7-B3-Diag-02_9_2] save raw logits:", B3_DIAG_SAVE_RAW_LOGITS)
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E7-B3-Diag-02_9_2 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E7-B3-Diag-02_9_2 test accuracy: {:.2f}. ----\n".format(final_acc))
