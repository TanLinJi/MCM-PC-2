#!/usr/bin/env python
"""
E7-A4-B2 runner: ULIP + ModelNet-C severity=2, candidate-pool top1 promotion.
...
2. 路由到 E7-A4-B2 的候选池 top1 晋升模型 run_test_tda；
...
from runners.E7_entropy_energy_alignment_multicache.model_e7_a4_b2_candidate_pool_top1_promotion import (
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"[E7-A4-B2] clip (final classifier) prompt source: {args.prompt_source}")
                print(f"[E7-A4-B2] text distribution prompt source: {args.e7_text_dist_prompt_source}")
                print(f"[E7-A4-B2] text_dist classes: {len(text_dist)}")
...
def main():
    ...
    args.e7_variant = "E7-A4-B2-candidate-pool-top1-promotion"
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E7-A4-B2 ULIP ModelNet-C severity=2 Candidate-Pool Top1 Promotion runner")
        ...
        print("Variant: E7-A4-B2 candidate-pool top1 promotion (global feature only, no local cache)")
