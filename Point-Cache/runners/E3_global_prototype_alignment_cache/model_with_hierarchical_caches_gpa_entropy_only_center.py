...
当前实现是 E3-V1-B-sequential-gpa-cache-entropy-only-center。
...
CENTER_SOURCE_LABEL = "Entropy-only center"
GPA_VARIANT_NAME = "E3-V1-B-sequential-gpa-cache-entropy-only-center"
...
def _compute_center_from_cache(cache, pred):
    """
    从单个 cache 中计算类别中心。
    cache[pred] 中每个 item[0] 是全局特征 pc_feats。
    """
    if cache is None or pred not in cache or len(cache[pred]) == 0:
        return None

    feats = [item[0] for item in cache[pred]]
...
def _compute_gpa_center(gpa_cache, pred, entropy_cache=None):
    ...
    类别 pred 的 GPA 原型中心不由 GPA Cache 自身计算，
    而是由原始 Global Entropy Cache 中该类别的全局特征均值得到。

    这样做的目的：
    - 保持顺序式 GPA Cache 关系不变；
    - 只改变“原型中心来源”；
    - 验证更稳定的 Global Entropy Cache 中心是否优于 GPA-only center。
    """
    return _compute_center_from_cache(entropy_cache, pred)
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if pos_enabled:
            ...
            accepted_entropy = _update_entropy_cache(
