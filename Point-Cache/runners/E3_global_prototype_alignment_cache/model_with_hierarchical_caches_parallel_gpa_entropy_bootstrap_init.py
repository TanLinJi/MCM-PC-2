...
当前实现是 E3-V2-C-parallel-gpa-cache-entropy-gpa-union-center。
...
CENTER_SOURCE_LABEL = "Entropy-bootstrap then Entropy+GPA union center"
GPA_VARIANT_NAME = "E3-V3-B-parallel-gpa-entropy-bootstrap-initialization"
...
def _sort_cache_and_local_together(cache, local_cache, pred):
    pairs = list(zip(cache[pred], local_cache[pred]))
    pairs = sorted(pairs, key=lambda pair: _loss_value(pair[0][1]))
    cache[pred] = [pair[0] for pair in pairs]
    local_cache[pred] = [pair[1] for pair in pairs]


def _update_entropy_cache_with_local(
    entropy_cache,
    entropy_local_cache,
    pred,
    global_item,
    ...
    """
    同步维护 Global Entropy Cache 及其对应 local item。

    Init-A 需要在 build 阶段结束后，
    用 Global Entropy Cache 初始化 GPA Cache，
    因此必须保存每个 entropy cache 样本对应的 local item。
    """
    curr_ent = _loss_value(global_item[1])

    if pred not in entropy_cache:
        entropy_cache[pred] = []
        entropy_local_cache[pred] = []

    if len(entropy_cache[pred]) < shot_capacity:
        entropy_cache[pred].append(global_item)
        entropy_local_cache[pred].append(local_item)
        _sort_cache_and_local_together(entropy_cache, entropy_local_cache, pred)
        stats[f"{phase}_entropy_add_with_local"] += 1
        return True

    worst_ent = _loss_value(entropy_cache[pred][-1][1])

    if curr_ent < worst_ent:
        entropy_cache[pred][-1] = global_item
        entropy_local_cache[pred][-1] = local_item
        _sort_cache_and_local_together(entropy_cache, entropy_local_cache, pred)
        stats[f"{phase}_entropy_replace_with_local"] += 1
        return True

    stats[f"{phase}_entropy_reject_with_local"] += 1
    return False


def _bootstrap_gpa_from_entropy_cache(
    entropy_cache,
    entropy_local_cache,
    gpa_cache,
    gpa_local_cache,
    ...
    """
    Init-A: Entropy-bootstrap initialization.

    build 阶段：
        只构建 Global Entropy Cache；
        不直接冷启动 GPA Cache。

    build 阶段结束后：
        用 Global Entropy Cache 中的低熵样本初始化 GPA Cache；
        同步把这些样本对应的 local item 写入 GPA-controlled Local Cache。

    test 阶段：
        继续使用 Entropy+GPA union center 做并列式 GPA 更新。
    """
    for pred, items in entropy_cache.items():
        if len(items) == 0:
            continue

        local_items = entropy_local_cache.get(pred, [])
        if len(local_items) != len(items):
            stats["build_entropy_bootstrap_local_mismatch"] += 1
            continue

        selected_global = list(items[:shot_capacity])
        selected_local = list(local_items[:len(selected_global)])

        gpa_cache[pred] = selected_global
        gpa_local_cache[pred] = selected_local
        _sort_cache_and_local_together(gpa_cache, gpa_local_cache, pred)

        stats["build_gpa_entropy_bootstrap_class"] += 1
        stats["build_gpa_entropy_bootstrap_items"] += len(selected_global)
        ...
        if event_records is not None:
            for item in selected_global:
                event_records.append({
                    "phase": "build",
                    "class_index": int(pred),
                    "decision": "entropy_bootstrap_selected",
                    "center_source": CENTER_SOURCE_LABEL,
                    "new_entropy": float(_loss_value(item[1])),
                    "old_entropy": None,
                    "new_distance": None,
                    "old_distance": None,
                })
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    include_prob_map=False:
        Init-A 正样本缓存构建：
        1. build 阶段只构建 Global Entropy Cache；
        2. 同步保存 Entropy Cache 样本对应的 local item；
        3. build 结束后用 Global Entropy Cache bootstrap GPA Cache；
        4. test 阶段继续使用 Entropy+GPA union center。
    ...
    print("*" * 10, "Building [global entropy] and [entropy-bootstrap GPA local] pos. cache ...", "*" * 10, "\n")
    ...
    for pc, _, _, rgb in test_loader:
        ...
        _update_entropy_cache_with_local(
            entropy_cache,
            entropy_local_cache,
            pred,
            global_item,
    ...
    _bootstrap_gpa_from_entropy_cache(
        entropy_cache,
        entropy_local_cache,
        gpa_cache,
        gpa_local_cache,
