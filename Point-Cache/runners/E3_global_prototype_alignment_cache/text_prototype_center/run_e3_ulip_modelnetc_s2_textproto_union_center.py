...
# 本 runner 用于在单个 Python 进程中复现 ULIP × ModelNet-C severity=2 的 7 个损坏设置，
# 避免 7 次重复加载模型。
import argparse
import csv
import gc
...
from runners.E3_global_prototype_alignment_cache.text_prototype_center.model_with_hierarchical_caches_textproto_union_center import run_test_tda as run_hierarchical_cache
...
def main():
    ...
    args.text_proto_visual_weight = float(os.environ.get("TEXT_PROTO_VISUAL_WEIGHT", "0.7"))
    args.text_proto_text_weight = float(os.environ.get("TEXT_PROTO_TEXT_WEIGHT", "0.3"))
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E3-V2-TextProto-C ULIP ModelNet-C severity=2 Text Prototype + visual union center runner")
        ...
        print(f"TEXT_PROTO_VISUAL_WEIGHT: {args.text_proto_visual_weight}")
        print(f"TEXT_PROTO_TEXT_WEIGHT: {args.text_proto_text_weight}")
