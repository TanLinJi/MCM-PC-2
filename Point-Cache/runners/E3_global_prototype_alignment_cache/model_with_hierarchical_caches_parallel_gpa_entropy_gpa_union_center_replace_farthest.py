...
当前实现是 E3-V2-Cb-parallel-gpa-cache-entropy-gpa-union-center-replace-farthest。
...
5. GPA Cache 形成中心后，再启用“低熵 + 小于当前最远距离”的严格更新；
...
CENTER_SOURCE_LABEL = "Entropy+GPA union center"
GPA_VARIANT_NAME = "E3-V2-Cb-parallel-gpa-cache-entropy-gpa-union-center-replace-farthest"
GPA_UPDATE_RULE = "low_entropy_gate_replace_farthest"
...
def _update_gpa_cache(entropy_cache, gpa_cache, gpa_local_cache, pred, global_item, local_item, shot_capacity, stats, phase, event_records=None):
    """
    ...
    2. GPA Cache 已满时：
       - 找到 GPA Cache 中当前最高熵样本，用它作为低熵门控阈值；
       - 找到 GPA Cache 中当前离联合中心最远的样本；
       - 如果新样本熵低于最高熵样本；
       - 并且新样本到联合中心的距离小于当前最远距离；
       - 则替换离中心最远的样本。
    """
    ...
    def record_event(
        decision,
        farthest_index=None,
        highest_entropy_index=None,
        old_entropy=None,
        highest_entropy=None,
        new_distance=None,
        old_distance=None,
        highest_entropy_distance=None,
        cached_entries=None,
    ):
        ...
        event_records.append({
            ...
            "highest_entropy": None if highest_entropy is None else float(highest_entropy),
            ...
            "highest_entropy_distance": None if highest_entropy_distance is None else float(highest_entropy_distance),
            "highest_entropy_is_farthest": (
                None
                if farthest_index is None or highest_entropy_index is None
                else int(farthest_index) == int(highest_entropy_index)
            ),
            "cached_entries": cached_entries,
        })

    # GPA Cache 未满：直接加入。
    # 并列式下，GPA Cache 不要求样本已经通过 Global Entropy Cache 准入。
    ...
    # GPA Cache 已满：启用 Entropy+GPA union center 的低熵门控 + 最远距离替换。
    center = _compute_gpa_center(gpa_cache, pred, entropy_cache=entropy_cache)
    ...
    cached_entries = [
        ...
        for idx, (entropy, distance) in enumerate(zip(cached_entropies, cached_distances))
    ]
    ...
    highest_ent_dist = cached_distances[highest_ent_idx]
    ...
    farthest_ent = cached_entropies[farthest_idx]

    entropy_pass = curr_ent < highest_ent
    distance_pass = curr_dist < farthest_dist

    if not entropy_pass:
        stats[f"{phase}_gpa_reject_entropy_gate"] += 1
        stats[f"{phase}_gpa_reject_low_entropy_farthest"] += 1
        record_event(
            decision="reject_entropy_gate",
            farthest_index=farthest_idx,
            highest_entropy_index=highest_ent_idx,
            old_entropy=farthest_ent,
            highest_entropy=highest_ent,
            new_distance=curr_dist,
            old_distance=farthest_dist,
            highest_entropy_distance=highest_ent_dist,
            cached_entries=cached_entries,
        )
        return False
    ...
    _sort_gpa_cache_and_local_together(gpa_cache, gpa_local_cache, pred)
    stats[f"{phase}_gpa_replace_low_entropy_farthest"] += 1
    stats[f"{phase}_gpa_center_update_after_replace"] += 1
    record_event(
        decision="replace_low_entropy_farthest",
        farthest_index=farthest_idx,
        highest_entropy_index=highest_ent_idx,
        old_entropy=farthest_ent,
        highest_entropy=highest_ent,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print("[E3-V2-Cb] len(entropy_cache):", len(entropy_cache))
    print("[E3-V2-Cb] len(gpa_cache):", len(gpa_cache))
    print("[E3-V2-Cb] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E3-V2-Cb] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E3-V2-Cb] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E3-V2-Cb] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    ...
    # 继承 build 阶段已经形成的 GPA global cache。
    # 新补实验必须保持 GPA-Cache 与 GPA-local-cache 一一对应，
    # 否则“替换最远样本”的索引无法解释。
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E3-V2-Cb test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E3-V2-Cb test accuracy: {:.2f}. ----\n".format(final_acc))
