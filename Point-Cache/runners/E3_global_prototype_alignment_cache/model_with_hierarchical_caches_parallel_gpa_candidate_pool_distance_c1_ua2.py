...
当前实现是 E3-V3-C1-Ua2：候选池距离初始化 GPA Cache。
...
GPA_VARIANT_NAME = "E3-V3-C1-Ua2-candidate-pool-distance-initialization"
...
def _rank_select_gpa_candidates(entropy_cache, gpa_candidate_pool, pred, shot_capacity):
    """
    E3-V3-C1-Ua2：从 2K 候选池中只按距离筛出 K 个样本。
...
def _update_gpa_cache(
    ...
    """
    E3-V3-C1-Ua2：候选池距离初始化 + 低熵门控 + 替换最远样本。
    ...
    正式阶段：
        使用低熵门控，但替换对象不是最高熵样本，而是离当前 GPA-Center 最远的样本。
        ...
        替换：
            replace farthest-to-center item
    ...
    def record_event(decision, farthest_index=None, highest_entropy_index=None, old_entropy=None, highest_entropy=None, new_distance=None, old_distance=None):
        if event_records is None:
            return
        event_records.append({
            "phase": phase,
            ...
            "decision": decision,
            "center_source": CENTER_SOURCE_LABEL,
            "update_rule": "low_entropy_gate_replace_farthest",
    ...
    record_event(
        decision="reject_low_entropy_farthest",
        farthest_index=farthest_idx,
        highest_entropy_index=highest_ent_idx,
        old_entropy=farthest_ent,
        highest_entropy=highest_ent,
...
def _save_gpa_stats(args, stats, entropy_cache, gpa_cache, gpa_local_cache, acc=None, event_records=None):
    ...
    if event_records is not None:
        ...
        print(f"[E3-V3-C1-Ua2] Saved GPA replacement events to {event_path}")

    print(f"[E3-V3-C1-Ua2] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        if gpa_cache_num == full_num:
            print("*" * 10, "Building [E3-V3-C1-Ua2 GPA/local] cache is full.", "*" * 10, "\n")
            break
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print("[E3-V3-C1-Ua2] len(entropy_cache):", len(entropy_cache))
    print("[E3-V3-C1-Ua2] len(gpa_cache):", len(gpa_cache))
    print("[E3-V3-C1-Ua2] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E3-V3-C1-Ua2] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E3-V3-C1-Ua2] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E3-V3-C1-Ua2] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E3-V3-C1-Ua2] gpa candidate pool total:", sum(len(v) for v in gpa_candidate_pool.values()))
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E3-V3-C1-Ua2 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E3-V3-C1-Ua2 test accuracy: {:.2f}. ----\n".format(final_acc))
