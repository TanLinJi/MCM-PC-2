...
当前实现是 E3-V3-A：并列式 GPA Cache + 候选池初始化。
...
CENTER_SOURCE_LABEL = "Entropy+GPA-candidate union center"
GPA_VARIANT_NAME = "E3-V3-A-parallel-gpa-candidate-pool-initialization"
...
def _compute_candidate_union_center(entropy_cache, candidate_pool, pred):
    ...
    E3-V3-A 初始化中心：
        ...
        center = mean(Global Entropy Cache[pred] ∪ GPA candidate pool[pred])

    这里的 candidate_pool 是还没有正式进入 GPA Cache 的候选样本。
    """
    feats = []
...
def _rank_select_gpa_candidates(entropy_cache, gpa_candidate_pool, pred, shot_capacity):
    ...
    排序依据：
        rank_score = entropy_rank + distance_rank

    这样可以避免直接设置 alpha/beta 权重，也避免熵和距离尺度不一致的问题。
    """
    ...
    entropy_order = sorted(rows, key=lambda x: (x["entropy"], x["distance"]))
    distance_order = sorted(rows, key=lambda x: (x["distance"], x["entropy"]))

    entropy_rank = {row["idx"]: rank for rank, row in enumerate(entropy_order)}
    distance_rank = {row["idx"]: rank for rank, row in enumerate(distance_order)}

    for row in rows:
        row["entropy_rank"] = entropy_rank[row["idx"]]
        row["distance_rank"] = distance_rank[row["idx"]]
        row["rank_score"] = row["entropy_rank"] + row["distance_rank"]

    rows = sorted(
        rows,
        key=lambda x: (x["rank_score"], x["entropy"], x["distance"])
    )
...
def _update_gpa_cache(
    ...
    """
    E3-V3-A：候选池初始化版本的 GPA Cache 更新。

    初始化阶段：
        如果该类 GPA Cache 尚未正式建立，
        则样本先进入 GPA candidate pool；
        当候选池达到 2K 时，
        使用 Entropy Cache ∪ GPA candidate pool 构造联合中心，
        再根据熵和距离筛出 K 个样本进入正式 GPA Cache；
        只有这 K 个样本的局部特征进入 local cache。

    正式阶段：
        如果该类 GPA Cache 已建立，
        则沿用 E3-V2-C 的并列式更新逻辑。
    """
    # 如果该类还没有形成“非空”的正式 GPA Cache，
    # 则仍然视为初始化阶段，继续进入 candidate pool。
    #
    # 注意：
    # 不能只判断 pred not in gpa_cache。
    # 在候选池初始化过程中，可能出现 pred 已经是 key，
    # 但 gpa_cache[pred] 仍为空列表的半初始化状态。
    ...
    if pred not in gpa_cache or len(gpa_cache.get(pred, [])) == 0:
        stats[f"{phase}_gpa_empty_formal_cache_reject"] += 1
        return False
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    # build 阶段结束后，对尚未达到 2K 但至少达到 K 的候选池进行补充初始化。
    for candidate_pred in list(gpa_candidate_pool.keys()):
        _finalize_gpa_candidate_pool(
            entropy_cache,
            gpa_cache,
            gpa_local_cache,
            gpa_candidate_pool,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print("[E3-GPA] gpa candidate pool total:", sum(len(v) for v in gpa_candidate_pool.values()))
