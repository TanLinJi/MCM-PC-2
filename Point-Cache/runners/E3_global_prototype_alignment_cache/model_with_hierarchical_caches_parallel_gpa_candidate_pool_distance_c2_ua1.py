...
当前实现是 E3-V3-C2-Ua1：候选池距离初始化 GPA Cache。
...
CENTER_SOURCE_LABEL = "Candidate+Entropy union center"
GPA_VARIANT_NAME = "E3-V3-C2-Ua1-candidate-pool-entropy-center-initialization"
...
def _compute_candidate_union_center(entropy_cache, candidate_pool, pred):
    ...
    E3-V3-C2 初始化中心：
        ...
        center = mean(EntropyCache[pred] ∪ GPA candidate pool[pred])

    注意：
        1. Entropy Cache 只参与候选池临时中心构造；
        2. 最终进入 GPA-Cache 的样本仍然只从 candidate pool 中选择；
        3. 这样可以验证 Entropy Cache 是否能作为 candidate-only center 的稳定锚点。
    """
    feats = []

    if entropy_cache is not None and pred in entropy_cache:
        feats.extend([item[0] for item in entropy_cache[pred]])
...
def _rank_select_gpa_candidates(entropy_cache, gpa_candidate_pool, pred, shot_capacity):
    """
    E3-V3-C2-Ua1：使用 Entropy Cache + 2K 候选池构造临时中心，并只按距离从候选池中筛出 K 个样本。

    规则：
        1. 临时中心由 entropy_cache[pred] 与 gpa_candidate_pool[pred] 共同构造；
        2. Entropy Cache 只参与中心构造，不参与候选样本排序；
        3. 选择距离 Candidate+Entropy 临时中心最近的 K 个候选样本进入 GPA-Cache。
    """
    candidates = gpa_candidate_pool.get(pred, [])
    if len(candidates) == 0:
        return [], []

    center = _compute_candidate_union_center(entropy_cache, gpa_candidate_pool, pred)
    if center is None:
        return [], []
...
def _update_gpa_cache(
    ...
    """
    E3-V3-C2-Ua1：候选池距离初始化 + 低熵门控 + 替换最高熵样本。

    初始化阶段：
        仍然使用 C1 规则：
        每类先收集 2K candidate pool；
        临时中心只由 candidate pool 构造；
        选择距离 Candidate+Entropy 临时中心最近的 K 个候选样本进入 GPA-Cache。

    正式阶段：
        对齐此前 E3-V2-C / MCP Align Cache 风格：
        新样本必须同时满足低熵和距离更近。
        ...
        替换：
            replace highest-entropy item

    每次替换后：
        同步替换 gpa_local_cache；
        立即重算 GPA-Center。
    """
    ...
    def record_event(decision, highest_entropy_index=None, old_entropy=None, new_distance=None, old_distance=None):
        if event_records is None:
            return
        event_records.append({
            "phase": phase,
            ...
            "decision": decision,
            "center_source": CENTER_SOURCE_LABEL,
            "update_rule": "low_entropy_gate_replace_highest_entropy",
            "highest_entropy_index": None if highest_entropy_index is None else int(highest_entropy_index),
    ...
    cached_entropies = [_loss_value(old_item[1]) for old_item in gpa_cache[pred]]
    highest_ent_idx = max(range(len(cached_entropies)), key=lambda idx: cached_entropies[idx])
    highest_ent = cached_entropies[highest_ent_idx]
    highest_ent_dist = _feature_distance_to_center(gpa_cache[pred][highest_ent_idx][0], center)

    entropy_pass = curr_ent < highest_ent
    distance_pass = curr_dist < highest_ent_dist

    if entropy_pass and distance_pass:
        gpa_cache[pred][highest_ent_idx] = global_item
        gpa_local_cache[pred][highest_ent_idx] = local_item
        stats[f"{phase}_gpa_replace_low_entropy_highest_entropy"] += 1

        record_event(
            decision="replace_low_entropy_highest_entropy",
            highest_entropy_index=highest_ent_idx,
            old_entropy=highest_ent,
            new_distance=curr_dist,
            old_distance=highest_ent_dist,
        )
    ...
    if not entropy_pass:
        stats[f"{phase}_gpa_reject_entropy_gate"] += 1
    elif not distance_pass:
        stats[f"{phase}_gpa_reject_distance_gate"] += 1
    stats[f"{phase}_gpa_reject_low_entropy_highest_entropy"] += 1

    record_event(
        decision="reject_low_entropy_highest_entropy",
        highest_entropy_index=highest_ent_idx,
        old_entropy=highest_ent,
        new_distance=curr_dist,
        old_distance=highest_ent_dist,
...
def _save_gpa_stats(args, stats, entropy_cache, gpa_cache, gpa_local_cache, acc=None, event_records=None):
    ...
    if event_records is not None:
        ...
        print(f"[E3-V3-C2-Ua1] Saved GPA replacement events to {event_path}")

    print(f"[E3-V3-C2-Ua1] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        if gpa_cache_num == full_num:
            print("*" * 10, "Building [E3-V3-C2-Ua1 GPA/local] cache is full.", "*" * 10, "\n")
            break
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print("[E3-V3-C2-Ua1] len(entropy_cache):", len(entropy_cache))
    print("[E3-V3-C2-Ua1] len(gpa_cache):", len(gpa_cache))
    print("[E3-V3-C2-Ua1] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E3-V3-C2-Ua1] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E3-V3-C2-Ua1] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E3-V3-C2-Ua1] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E3-V3-C2-Ua1] gpa candidate pool total:", sum(len(v) for v in gpa_candidate_pool.values()))
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E3-V3-C2-Ua1 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E3-V3-C2-Ua1 test accuracy: {:.2f}. ----\n".format(final_acc))
