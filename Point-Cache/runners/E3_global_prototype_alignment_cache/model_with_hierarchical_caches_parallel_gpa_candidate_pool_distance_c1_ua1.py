...
当前实现是 E3-V3-C1-Ua1：候选池距离初始化 GPA Cache。
...
GPA_VARIANT_NAME = "E3-V3-C1-Ua1-candidate-pool-distance-initialization"
...
def _rank_select_gpa_candidates(entropy_cache, gpa_candidate_pool, pred, shot_capacity):
    """
    E3-V3-C1-Ua1：从 2K 候选池中只按距离筛出 K 个样本。
...
def _update_gpa_cache(
    ...
    """
    E3-V3-C1-Ua1：候选池距离初始化 + 低熵门控 + 替换最高熵样本。

    初始化阶段：
        仍然使用 C1 规则：
        每类先收集 2K candidate pool；
        临时中心只由 candidate pool 构造；
        选择距离临时中心最近的 K 个样本进入 GPA-Cache。
...
def _save_gpa_stats(args, stats, entropy_cache, gpa_cache, gpa_local_cache, acc=None, event_records=None):
    ...
    if event_records is not None:
        ...
        print(f"[E3-V3-C1-Ua1] Saved GPA replacement events to {event_path}")

    print(f"[E3-V3-C1-Ua1] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        if gpa_cache_num == full_num:
            print("*" * 10, "Building [E3-V3-C1-Ua1 GPA/local] cache is full.", "*" * 10, "\n")
            break
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    print("[E3-V3-C1-Ua1] len(entropy_cache):", len(entropy_cache))
    print("[E3-V3-C1-Ua1] len(gpa_cache):", len(gpa_cache))
    print("[E3-V3-C1-Ua1] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E3-V3-C1-Ua1] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E3-V3-C1-Ua1] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E3-V3-C1-Ua1] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E3-V3-C1-Ua1] gpa candidate pool total:", sum(len(v) for v in gpa_candidate_pool.values()))
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E3-V3-C1-Ua1 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E3-V3-C1-Ua1 test accuracy: {:.2f}. ----\n".format(final_acc))
