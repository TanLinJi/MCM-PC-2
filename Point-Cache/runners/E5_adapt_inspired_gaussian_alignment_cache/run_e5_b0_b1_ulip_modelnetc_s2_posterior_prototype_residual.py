...
from runners.E5_adapt_inspired_gaussian_alignment_cache.model_e5_b0_b1_posterior_prototype_residual import (
...
def get_selected_corruptions():
    raw = os.environ.get("E5_CORRUPTIONS", "").strip()
    if not raw:
        return list(CORRUPTIONS)

    selected = [part.strip() for part in raw.split(",") if part.strip()]
    invalid = [item for item in selected if item not in CORRUPTIONS]
    if invalid:
        raise ValueError(f"Unsupported E5_CORRUPTIONS values: {invalid}. Supported: {CORRUPTIONS}")
    return selected
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"[E5-B0/B1] baseline clip prompt source: {args.prompt_source}")
                print(f"[E5-B0/B1] E4 text distribution prompt source: {args.e4_text_dist_prompt_source}")
                print(f"[E5-B0/B1] text_dist classes: {len(text_dist)}")
                ...
                if baseline_method == "zs":
                    ...
                    acc = run_hierarchical_cache(args, cfg["positive"], cfg["negative"], test_loader, lm3d_model, clip_weights, text_dist=text_dist)
                else:
                    raise ValueError(f"Unsupported baseline_method: {baseline_method}")

                if args.wandb:
                    if isinstance(acc, dict):
                        wandb.log({f"{args.dataset}/{k}": float(v) for k, v in acc.items()})
                    else:
                ...
                if isinstance(acc, dict):
                    return {str(k): float(v) for k, v in acc.items()}
                return {"original": float(acc)}
...
def main():
    ...
    args.e4_variant = "E4-C-A0+E1-textdist-only"
    args.e5_variant = "E5-B0/B1-posterior-prototype-residual"
    ...
    args.e5_statsbank_capacity = int(os.environ.get("E5_STATSBANK_CAPACITY", "16"))
    args.e5_posterior_kappa = float(os.environ.get("E5_POSTERIOR_KAPPA", "16"))
    args.e5_posterior_min_total = int(os.environ.get("E5_POSTERIOR_MIN_TOTAL", "8"))
    args.e5_posterior_min_classes = int(os.environ.get("E5_POSTERIOR_MIN_CLASSES", "2"))
    args.e5_posterior_norm_eps = float(os.environ.get("E5_POSTERIOR_NORM_EPS", "1e-6"))
    args.e5_posterior_norm_clip = float(os.environ.get("E5_POSTERIOR_NORM_CLIP", "3.0"))
    args.e5_posterior_gammas = os.environ.get("E5_POSTERIOR_GAMMAS", "0.05,0.10,0.20")
    args.e5_save_sample_diagnostics = os.environ.get("E5_SAVE_SAMPLE_DIAGNOSTICS", "1")
    selected_corruptions = get_selected_corruptions()
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E5-B0/B1 ULIP ModelNet-C severity=2 posterior prototype residual runner")
        ...
        print("Variant: E5-B0/B1 posterior residual over E4-C-A0+E1-textdist-only baseline")
        ...
        print(f"E5_STATSBANK_CAPACITY: {args.e5_statsbank_capacity}")
        print(f"E5_POSTERIOR_KAPPA: {args.e5_posterior_kappa}")
        print(f"E5_POSTERIOR_MIN_TOTAL: {args.e5_posterior_min_total}")
        print(f"E5_POSTERIOR_MIN_CLASSES: {args.e5_posterior_min_classes}")
        print(f"E5_POSTERIOR_NORM_EPS: {args.e5_posterior_norm_eps}")
        print(f"E5_POSTERIOR_NORM_CLIP: {args.e5_posterior_norm_clip}")
        print(f"E5_POSTERIOR_GAMMAS: {args.e5_posterior_gammas}")
        print(f"E5_SAVE_SAMPLE_DIAGNOSTICS: {args.e5_save_sample_diagnostics}")
        print(f"E5_CORRUPTIONS: {selected_corruptions}")
        print(f"Model will be loaded once, then {len(selected_corruptions)} severity=2 cor_type values will be evaluated.")
        print("============================================================")
    ...
    for corruption in selected_corruptions:
        for severity in SEVERITIES:
            ...
            try:
                ...
                for result_key, result_acc in result.items():
                    if result_key == "original":
                        row_method_full = f"{method_full} | original_pointcache_formula"
                    else:
                        row_method_full = f"{method_full} | {result_key}"
