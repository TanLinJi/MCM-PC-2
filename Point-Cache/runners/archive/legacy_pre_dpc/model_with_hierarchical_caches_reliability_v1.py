...
Reliability Score v1 for positive cache admission.
...
updates positive cache entries by a reliability score:
    ...
    R(x) = -H(x) + lambda_margin * M(x)

where H(x) is the normalized entropy and M(x) is the top-1 / top-2
probability margin.
...
@torch.no_grad()
def compute_reliability_score(loss, prob_map, clip_weights, lambda_margin=1.0):
    """Compute reliability score for positive cache admission.

    Original Point-Cache uses entropy as the main quality criterion. In this
    Stage-2 version, we additionally consider the classification margin:
    ...
    Args:
        ...
        clip_weights: Text classifier weights, used by get_entropy().
        lambda_margin: Weight for the margin term.
    ...
    reliability = -entropy + lambda_margin * margin

    return reliability, entropy, margin
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        if include_prob_map:
            ...
            # Stage 2 change: positive cache uses reliability score.
            reliability, entropy, margin = compute_reliability_score(
                loss, prob_map, clip_weights, lambda_margin=1.0
            )
            item = [pc_feats, loss, reliability, entropy, margin]
            local_item = [patch_centers, loss, reliability, entropy, margin]
...
def update_cache(cache, local_cache, pred, features_loss, shot_capacity, include_prob_map=False):
    ...
    Positive cache input:
        features_loss = [pc_feats, patch_centers, loss, reliability, entropy, margin]
        cache item    = [pc_feats, loss, reliability, entropy, margin]
        local item    = [patch_centers, loss, reliability, entropy, margin]
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights):
    ...
    Stage 2 changes are restricted to positive cache admission and update.
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if pos_enabled:
            ...
            update_cache(
                ...
                pos_local_cache,
                pred,
                [pc_feats, patch_centers, loss, reliability, entropy, margin],
                pos_params['shot_capacity']
            )
