"""
E4-C-A1: Dual-Low Trusted Fallback for Distribution-Guided GPA Cache.
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual class-wise distribution score with dual-low trusted fallback"
GPA_VARIANT_NAME = "E4-C-A1-dual-low-trusted-fallback-accepted-history-text-visual-distribution-guided-gpa-cache"
...
def _save_gpa_stats(
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A1] Saved GPA replacement events to {event_path}")

    print(f"[E4-C-A1] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # E4-C-A1：可信样本优先使用双低信号，否则回到历史 visual_dist + text_dist 评分。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
    ...
    return entropy_cache, gpa_cache, gpa_local_cache, visual_dist, score_norm_state, energy_stats_state, stats, gpa_event_records
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    Negative cache:
        保持原始 Point-Cache 逻辑。
    """
    entropy_cache, gpa_cache, gpa_local_cache, visual_dist, score_norm_state, energy_stats_state, build_stats, gpa_event_records = build_cache_in_advance(
        args, test_loader, lm3d_model, clip_weights, pos_cfg["shot_capacity"], text_dist=text_dist
    )

    print("[E4-C-A1] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A1] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A1] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A1] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A1] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A1] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A1] visual distribution classes:", len(visual_dist))
    print("[E4-C-A1] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A1] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A1] score norm state:", _summarize_score_norm_state(score_norm_state))
    print("[E4-C-A1] trusted fallback mode:", TRUSTED_FALLBACK_MODE)
    print("[E4-C-A1] trusted entropy threshold:", TRUSTED_ENTROPY_THRESHOLD)
    print("[E4-C-A1] trusted energy z threshold:", TRUSTED_ENERGY_Z_THRESHOLD)
    print("[E4-C-A1] trusted energy min count:", TRUSTED_ENERGY_MIN_COUNT)
    print("[E4-C-A1] energy stats state:", _summarize_energy_stats_state(energy_stats_state))
    ...
    # E4-C-A1 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E4-C-A1 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E4-C-A1 test accuracy: {:.2f}. ----\n".format(final_acc))
