...
def default_exp_id(setting):
    return (
        f"09_1_{setting}_all35_ulip_modelnetc_zs_global_local_"
        "e4_c_a0_e1_decoupled_cache_capacity_tw0p15_score_norm_"
        "manual_full_clip_manualfull_llm_dynamic_init_textdist"
    )


def build_method_full(args):
    return (
        f"09_1 all35 {args.setting}: 02_9_2 decoupled cache-capacity ablation; "
        f"entropy_cap={args.entropy_cap}; gpa_cap={args.gpa_cap}; "
        f"local_cap={args.local_cap}; neg_cap={args.neg_cap}; "
        f"local_centers={args.local_centers}; text_weight={args.text_weight}; "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run 09_1 decoupled cache-capacity settings on all 35 ModelNet-C corruptions."
    )
    parser.add_argument("--setting", required=True)
    parser.add_argument("--entropy-cap", required=True, type=int)
    parser.add_argument("--gpa-cap", required=True, type=int)
    parser.add_argument("--local-cap", required=True, type=int)
    parser.add_argument("--neg-cap", required=True, type=int)
    parser.add_argument("--local-centers", default=3, type=int)
    parser.add_argument("--exp-id", default=None)
    parser.add_argument("--gpus", default="0", help="Single physical GPU id. Use 0 on a one-card 4090 machine.")
    ...
    for name in ["entropy_cap", "gpa_cap", "local_cap", "neg_cap", "local_centers"]:
        value = int(getattr(args, name))
        if value <= 0:
            raise SystemExit(f"{name.replace('_', '-')} must be positive, got {value}")

    if args.exp_id is None:
        args.exp_id = default_exp_id(args.setting)
...
def prompt_cache_is_complete(cache_path, class_file, required_prompt_count=10):
    ...
    if missing or short:
        lines = ["shared E1 prompt cache is incomplete; stop to avoid changing the 09_1 setting."]
        if missing:
...
def build_worker_command(args, worker_id, gpu_id, task_file):
    worker_script = Path(__file__).resolve().parent / "worker_09_1_modelnetc_all35_decoupled_cache_capacity.py"
    return [
        args.python,
        str(worker_script),
        "--worker-id",
...
def build_worker_env(args, worker_id, gpu_id):
    env = os.environ.copy()
    ...
    env["E4_ENTROPY_CAP"] = str(args.entropy_cap)
    env["E4_GPA_CAP"] = str(args.gpa_cap)
    env["E4_LOCAL_CAP"] = str(args.local_cap)
    env["E4_NEG_CAP"] = str(args.neg_cap)
    env["E4_LOCAL_CENTERS"] = str(args.local_centers)
    return env
...
def run_workers(args, run_dir, task_files, gpu_ids):
    ...
    for worker_id, (gpu_id, task_file) in enumerate(zip(gpu_ids, task_files)):
        ...
        env = build_worker_env(args, worker_id, gpu_id)
...
def write_table_html(run_dir, args, table, severity_avgs, overall_avg):
    ...
    title = f"09_1 {html.escape(args.setting)} ModelNet-C All35 Results"
    meta = (
        f"entropy={args.entropy_cap}, gpa={args.gpa_cap}, local={args.local_cap}, "
        f"negative={args.neg_cap}, local_centers={args.local_centers}, "
        f"text weight {html.escape(str(args.text_weight))}, running z-score normalization."
    )
    page = f"""<!doctype html>
<html lang="zh-CN">
<head>
...
def merge_and_write_tables(run_dir, args):
    ...
    html_table = write_table_html(run_dir, args, table, severity_avgs, overall_avg)
...
def main():
    ...
    if len(gpu_ids) != 1:
        raise SystemExit("09_1 all35 is configured for a single 4090 GPU. Please pass one GPU id, for example --gpus 0.")
    ...
    print("============================================================")
    print("09_1 ModelNet-C all35 decoupled cache-capacity launcher")
    ...
    print(f"Capacity: entropy={args.entropy_cap}, gpa={args.gpa_cap}, local={args.local_cap}, neg={args.neg_cap}")
    print(f"Local centers: {args.local_centers}")
    print(f"Task split: {', '.join(str(len(chunk)) for chunk in task_chunks)}")
    print("Execution model: one worker process on the selected GPU; the worker loads the model once.")
    print("============================================================")
    ...
    summary_file, csv_table, md_table, html_table = merge_and_write_tables(run_dir, args)
