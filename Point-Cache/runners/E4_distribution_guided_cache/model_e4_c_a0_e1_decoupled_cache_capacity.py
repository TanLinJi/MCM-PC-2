"""
09_1: Decoupled cache-capacity variant of 02_9_2 / E4-C-A0+E1-textdist-only.
...
8. 仅解耦四类缓存的样本容量，不修改 gate、score 或最终预测加权公式。
"""
...
GPA_VARIANT_NAME = "09_1-decoupled-cache-capacity-e4-c-a0-e1-textdist-only"
...
def _resolve_cache_caps(args, pos_cfg, neg_cfg):
    """
    Resolve explicit 09_1 cache capacities.
...
def _update_local_cache_from_gpa_accept(local_cache, pred, local_item, local_cap, stats, phase):
    ...
    experiments, but first-batch 09_1 settings keep local_cap == gpa_cap.
    """
    if local_cap <= 0:
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    print("[09_1] decoupled cache capacities:", capacity_summary)
    ...
    if pos_enabled:
        pos_params = {k: pos_cfg[k] for k in ["alpha", "beta"]}
    if neg_enabled:
        neg_params = {k: neg_cfg[k] for k in ["alpha", "beta", "entropy_threshold", "mask_threshold"]}
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        zero_pred = _diag_record_prediction_stage(gpa_cache_stats, "test", "zero_shot", final_logits, target)
        ...
        if pos_enabled and entropy_cache:
            ...
            entropy_pred = _diag_record_prediction_stage(gpa_cache_stats, "test", "after_entropy", final_logits, target)
            ...
            if gpa_local_cache:
                ...
                local_pred = _diag_record_prediction_stage(gpa_cache_stats, "test", "after_local", final_logits, target)
                _diag_record_prediction_transition(
        ...
        if neg_enabled and neg_cache:
            ...
            neg_pred = _diag_record_prediction_stage(gpa_cache_stats, "test", "after_negative", final_logits, target)
            _diag_record_prediction_transition(
        ...
        final_pred = _diag_record_prediction_stage(gpa_cache_stats, "test", "final", final_logits, target)
        _diag_record_prediction_transition(
