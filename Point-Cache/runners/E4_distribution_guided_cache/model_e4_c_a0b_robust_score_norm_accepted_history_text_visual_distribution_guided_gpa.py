...
6. GPA Cache 满后使用“低熵 + robust-normalized text-visual joint score 更高”替换最高熵样本；
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual robust-normalized class-wise distribution score"
GPA_VARIANT_NAME = "E4-C-A0b-robust-score-normalized-accepted-history-text-visual-distribution-guided-gpa-cache"
...
# ============================================================
# E4-C-A0b：Robust Score-Normalized Accepted-History Text-Visual 类别概率分布辅助函数
# ============================================================
...
if SCORE_NORM_MODE not in {"none", "running_zscore", "running_robust_iqr"}:
...
def _make_score_norm_state():
    return {
        "visual": {"count": 0, "mean": 0.0, "m2": 0.0, "values": [], "robust_cache": None},
        "text": {"count": 0, "mean": 0.0, "m2": 0.0, "values": [], "robust_cache": None},
    }
...
def _quantile_from_sorted(values, q):
    if not values:
        return None
    if len(values) == 1:
...
def _running_robust_stats(entry):
    count = int(entry["count"])
    if count < 2:
        return None

    cache = entry.get("robust_cache")
    if cache is not None and int(cache.get("count", -1)) == count:
        return cache

    values = sorted(float(v) for v in entry["values"])
    q1 = _quantile_from_sorted(values, 0.25)
    median = _quantile_from_sorted(values, 0.5)
    q3 = _quantile_from_sorted(values, 0.75)
    ...
    cache = {
        ...
        "iqr": None if iqr is None else float(iqr),
    }
    entry["robust_cache"] = cache
    return cache
...
def _score_for_joint(score_norm_state, modality, raw_score):
    ...
    if SCORE_NORM_MODE == "running_zscore":
    ...
    elif SCORE_NORM_MODE == "running_robust_iqr":
        robust = _running_robust_stats(entry)
        if (
            count < SCORE_NORM_MIN_COUNT
            or robust is None
            or robust["median"] is None
            or robust["iqr"] is None
            or robust["iqr"] < SCORE_NORM_EPS
        ):
            return float(raw_score), False
        score = (float(raw_score) - float(robust["median"])) / (float(robust["iqr"]) + SCORE_NORM_EPS)
    else:
        return float(raw_score), False
...
def _score_norm_ready(score_norm_state, modalities):
    ...
    for modality in modalities:
        entry = score_norm_state[modality]
        count = int(entry["count"])
        if count < SCORE_NORM_MIN_COUNT:
            return False

        if SCORE_NORM_MODE == "running_zscore":
            std = _running_std(entry)
            if std is None or std < SCORE_NORM_EPS:
                return False
        elif SCORE_NORM_MODE == "running_robust_iqr":
            robust = _running_robust_stats(entry)
            if robust is None or robust["iqr"] is None or robust["iqr"] < SCORE_NORM_EPS:
                return False
        else:
            return False
...
def _summarize_score_norm_state(score_norm_state):
    ...
    for modality, entry in score_norm_state.items():
        ...
        summary[modality] = {
            ...
            "median": None if robust is None else robust["median"],
            "q1": None if robust is None else robust["q1"],
            "q3": None if robust is None else robust["q3"],
            "iqr": None if robust is None else robust["iqr"],
        }
    return summary
...
def _joint_distribution_score(visual_dist, text_dist, pred, feat, score_norm_state=None):
    """
    计算 E4-C-A0b 的 text-visual joint score。
...
def _update_gpa_cache(
    ...
    """
    E4-C-A0b：Robust Score-Normalized Accepted-History Text-Visual 类别概率分布引导的 GPA-Cache 更新。
...
def _save_gpa_stats(
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A0b] Saved GPA replacement events to {event_path}")

    print(f"[E4-C-A0b] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # E4-C-A0b：GPA 使用已被正缓存接受过的历史可信 visual_dist 评分。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    print("[E4-C-A0b] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A0b] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A0b] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A0b] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A0b] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A0b] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A0b] visual distribution classes:", len(visual_dist))
    print("[E4-C-A0b] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A0b] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A0b] score norm state:", _summarize_score_norm_state(score_norm_state))
    ...
    # E4-C-A0b 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E4-C-A0b test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E4-C-A0b test accuracy: {:.2f}. ----\n".format(final_acc))
