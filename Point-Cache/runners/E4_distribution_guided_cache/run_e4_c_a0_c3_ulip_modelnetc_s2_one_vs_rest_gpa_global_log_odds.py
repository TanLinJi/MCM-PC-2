...
from runners.E4_distribution_guided_cache.model_e4_c_a0_c3_one_vs_rest_gpa_global_log_odds import (
    get_formula_labels,
    get_formula_summary_slug,
...
def rebuild_combined_summary(summary_file, summary_files):
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"[E4-C-A0-c3] text_dist classes: {len(text_dist)}")
                ...
                if args.wandb:
                    if isinstance(acc, dict):
                        wandb.log({
                            f"{args.dataset}/{formula}": float(acc[formula])
                            for formula in get_formula_labels()
                        })
                    else:
                ...
                if isinstance(acc, dict):
                    metrics = ", ".join(
                        f"{formula}={float(acc[formula]):.2f}"
                        for formula in get_formula_labels()
                    )
                    print(f"DONE: {cor_type}, {metrics}")
                else:
...
def main():
    ...
    summary_files = {
        formula: run_dir / f"summary_{get_formula_summary_slug(formula)}.csv"
    ...
    for formula in formula_labels:
        write_summary_header(summary_files[formula])
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E4-C-A0-c3 ULIP ModelNet-C severity=2 Z-score Accepted-History Text-Visual GPA Cache One-vs-Rest GPA Global Log-Odds runner")
        ...
        print(f"E4_GPA_GLOBAL_PRIOR: {os.environ.get('E4_GPA_GLOBAL_PRIOR', os.environ.get('GPA_GLOBAL_PRIOR', 'uniform'))}")
        print(f"E4_GPA_GLOBAL_EPS: {os.environ.get('E4_GPA_GLOBAL_EPS', os.environ.get('GPA_GLOBAL_EPS', '1e-12'))}")
        print(f"E4_GPA_GLOBAL_BETA: {os.environ.get('E4_GPA_GLOBAL_BETA', os.environ.get('GPA_GLOBAL_BETA', 'positive.beta'))}")
        print(f"E4_GPA_GLOBAL_PRIOR_GAMMAS: {os.environ.get('E4_GPA_GLOBAL_PRIOR_GAMMAS', '0.1,0.25,0.5')}")
        print(f"E4_GPA_GLOBAL_ODDS_GAMMAS: {os.environ.get('E4_GPA_GLOBAL_ODDS_GAMMAS', os.environ.get('E4_GPA_GLOBAL_GAMMAS', os.environ.get('GPA_GLOBAL_GAMMAS', '0.05,0.1,0.25')))}")
        print(f"Formula labels: {formula_labels}")
        ...
        for severity in SEVERITIES:
            ...
            if not data_file.exists():
                for formula in formula_labels:
                    ...
                    append_summary_row(summary_files[formula], missing_row)
                rebuild_combined_summary(summary_file, [summary_files[formula] for formula in formula_labels])
            ...
            try:
                ...
                for formula in formula_labels:
                    formula_acc = float(acc[formula]) if isinstance(acc, dict) else float(acc)
                    append_summary_row(summary_files[formula], [
                        exp_id,
                        args.dataset,
                        data_root,
                        corruption,
                ...
                for formula in formula_labels:
                    ...
                    append_summary_row(summary_files[formula], failed_row)
                rebuild_combined_summary(summary_file, [summary_files[formula] for formula in formula_labels])
                raise
    ...
    for formula in formula_labels:
        print(f"summary_{get_formula_summary_slug(formula)}: {summary_files[formula]}")
    print("============================================================")
