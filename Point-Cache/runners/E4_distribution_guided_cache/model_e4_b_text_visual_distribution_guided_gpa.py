"""
E4-B: Text-Visual Distribution-Guided GPA Cache for Point-Cache.
...
4. 视觉端从当前 EntropyCache[c] ∪ GPACache[c] 动态重建 visual distribution；
...
CENTER_SOURCE_LABEL = "Text-visual class-wise distribution score"
GPA_VARIANT_NAME = "E4-B-text-visual-distribution-guided-gpa-cache"
...
def _collect_unique_cache_features(entropy_cache, gpa_cache, pred):
    """
    从当前 EntropyCache[c] ∪ GPACache[c] 提取去重后的视觉特征。

    注意：
        这里不使用历史累加统计。cache 中被替换掉的旧样本不会继续参与分布估计。
    """
    pred = int(pred)
    features = []
    seen = set()

    for cache in (entropy_cache, gpa_cache):
        for item in cache.get(pred, []):
            feat = item[0]
            key = _feature_key(feat)
            if key in seen:
                continue
            seen.add(key)
            features.append(_feature_float(feat))

    return features


def _distribution_from_features(features, min_var):
    if len(features) < 2:
        return None

    stacked = torch.cat(features, dim=0).float()
    mean = stacked.mean(dim=0, keepdim=True)
    var = stacked.var(dim=0, unbiased=True, keepdim=True).clamp_min(min_var)
...
def _visual_distribution(entropy_cache, gpa_cache, pred):
    features = _collect_unique_cache_features(entropy_cache, gpa_cache, pred)
    return _distribution_from_features(features, DIST_MIN_VAR)
...
def _joint_distribution_score(entropy_cache, gpa_cache, text_dist, pred, feat):
    """
    计算 E4-B 的 text-visual joint score。

    visual_dist 来自当前 EntropyCache[c] ∪ GPACache[c]；
    text_dist 来自固定 prompt-level embeddings。
    """
    visual_entry = _visual_distribution(entropy_cache, gpa_cache, pred)
...
def _summarize_visual_distribution(entropy_cache, gpa_cache):
    summary = {}
    class_ids = set(entropy_cache.keys()) | set(gpa_cache.keys())

    for c in sorted(class_ids):
        entry = _visual_distribution(entropy_cache, gpa_cache, c)
        if entry is not None:
            summary[str(c)] = _summarize_distribution_from_entry(entry)
        else:
            count = len(_collect_unique_cache_features(entropy_cache, gpa_cache, c))
...
def _update_gpa_cache(entropy_cache, gpa_cache, gpa_local_cache, text_dist, pred, global_item, local_item, shot_capacity, stats, phase, event_records=None):
    """
    E4-B：Text-Visual 类别概率分布引导的 GPA-Cache 更新。
    ...
    改动：
        用当前 EntropyCache[c] ∪ GPACache[c] 的视觉分布，以及固定 prompt 文本分布，
        共同计算 joint score。
    """
    ...
    def record_event(decision, old_entropy=None, new_score=None, old_score=None):
        ...
        event_records.append({
            ...
            "decision": decision,
            "update_rule": "low_entropy_gate_text_visual_joint_score_replace_highest_entropy",
    ...
    if len(gpa_cache[pred]) < shot_capacity:
        ...
        stats[f"{phase}_gpa_add_not_full_text_visual_distribution"] += 1
        record_event(decision="add_not_full_text_visual_distribution")
        return True

    worst_global_item = gpa_cache[pred][-1]
    worst_ent = _loss_value(worst_global_item[1])

    curr_score = _joint_distribution_score(entropy_cache, gpa_cache, text_dist, pred, global_item[0])
    worst_score = _joint_distribution_score(entropy_cache, gpa_cache, text_dist, pred, worst_global_item[0])

    if curr_score is None or worst_score is None:
        stats[f"{phase}_gpa_reject_no_text_visual_distribution"] += 1
        record_event(decision="reject_no_text_visual_distribution", old_entropy=worst_ent, new_score=curr_score, old_score=worst_score)
        return False

    if curr_ent >= worst_ent:
        stats[f"{phase}_gpa_reject_entropy"] += 1
        stats[f"{phase}_gpa_reject_entropy_text_visual_distribution"] += 1
        record_event(decision="reject_entropy_text_visual_distribution", old_entropy=worst_ent, new_score=curr_score, old_score=worst_score)
        return False
    ...
    if curr_score["joint"] > worst_score["joint"]:
        ...
        stats[f"{phase}_gpa_replace_text_visual_distribution"] += 1
        record_event(decision="replace_text_visual_distribution", old_entropy=worst_ent, new_score=curr_score, old_score=worst_score)
        return True

    stats[f"{phase}_gpa_reject_text_visual_distribution"] += 1
    record_event(decision="reject_text_visual_distribution", old_entropy=worst_ent, new_score=curr_score, old_score=worst_score)
    return False
...
def _save_gpa_stats(args, stats, entropy_cache, gpa_cache, gpa_local_cache, text_dist=None, acc=None, event_records=None):
    ...
    payload = {
        ...
        "visual_distribution_scope": "current_entropy_cache_union_gpa_cache",
        "visual_distribution_history_policy": "current_cache_state_only_no_historical_accumulation",
        ...
        "visual_distribution_num_classes": int(len(set(entropy_cache.keys()) | set(gpa_cache.keys()))),
        "visual_distribution_summary": _summarize_visual_distribution(entropy_cache, gpa_cache),
    ...
    if event_records is not None:
        ...
        print(f"[E4-B] Saved GPA replacement events to {event_path}")

    print(f"[E4-B] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # 与 test 阶段保持一致：先用当前旧缓存评分 GPA，再更新 entropy cache。
        # 这样候选样本不会通过 EntropyCache 进入自己的 visual_dist。
        _update_gpa_cache(
            entropy_cache,
            gpa_cache,
            gpa_local_cache,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    print("[E4-B] len(entropy_cache):", len(entropy_cache))
    print("[E4-B] len(gpa_cache):", len(gpa_cache))
    print("[E4-B] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-B] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-B] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-B] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-B] text distribution classes:", 0 if text_dist is None else len(text_dist))
    ...
    # E4-B 保留 build 阶段形成的 EntropyCache、GPA-Cache 和 GPA-local-cache。
    # visual_dist 每次从当前 EntropyCache[c] ∪ GPACache[c] 动态重建，不累加历史样本。
    accuracies = []
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if pos_enabled:
            ...
            # 先用旧的 EntropyCache ∪ GPACache 评分 GPA，避免当前候选样本通过
            # EntropyCache 进入自己的 visual_dist，造成自我增强。
            _update_gpa_cache(
                entropy_cache,
                gpa_cache,
                gpa_local_cache,
        ...
        if i % args.print_freq == 0:
            print("---- E4-B test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E4-B test accuracy: {:.2f}. ----\n".format(final_acc))

    _save_gpa_stats(args, gpa_cache_stats, entropy_cache, gpa_cache, gpa_local_cache, text_dist, final_acc, gpa_event_records)
