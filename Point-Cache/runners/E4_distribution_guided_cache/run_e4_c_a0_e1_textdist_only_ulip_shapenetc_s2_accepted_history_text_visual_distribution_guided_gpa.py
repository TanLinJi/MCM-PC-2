...
def build_clip_weights_once(args, clip_model, clip_weights_state, classnames, template):
    if clip_weights_state["clip_weights"] is None:
        ...
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                "Classnames changed inside ShapeNet-C severity=2 loop. "
                "clip_weights must be rebuilt, but this runner expects fixed ShapeNet-C classes."
            )
...
def build_text_distribution_once(args, clip_model, clip_weights_state, classnames, template):
    if clip_weights_state.get("text_dist") is not None:
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                "Classnames changed inside ShapeNet-C severity=2 loop. "
                "text_dist must be rebuilt, but this runner expects fixed ShapeNet-C classes."
            )
...
def main():
    ...
    # ShapeNet-C files are stored under data/shapenet_c, while the existing
    # dataset loader for this benchmark is named snv2_c.
    args.dataset = "snv2_c"
    args.snv2_c_root = "data/shapenet_c"
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E4-C-A0+E1-textdist ULIP ShapeNet-C severity=2 Accepted-History Text-Visual Distribution-Guided GPA Cache runner")
