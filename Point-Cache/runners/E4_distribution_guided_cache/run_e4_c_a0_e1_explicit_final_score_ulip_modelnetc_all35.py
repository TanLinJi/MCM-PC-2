...
def build_clip_weights_once(args, clip_model, clip_weights_state, classnames, template):
    if clip_weights_state["clip_weights"] is None:
        ...
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                "Classnames changed inside ModelNet-C all35 loop. "
...
def main():
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("09_2 E4-C-A0+E1-textdist ULIP ModelNet-C all35 Explicit Final-Score runner")
        ...
        print(f"Model will be loaded once, then {len(CORRUPTIONS) * len(SEVERITIES)} ModelNet-C cor_type values will be evaluated.")
        print("============================================================")
    ...
    print("============================================================")
    print(f"All {len(CORRUPTIONS) * len(SEVERITIES)} ModelNet-C all35 runs finished: {exp_id}")
