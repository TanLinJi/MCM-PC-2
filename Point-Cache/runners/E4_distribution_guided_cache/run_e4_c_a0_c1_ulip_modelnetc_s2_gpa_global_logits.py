...
from runners.E4_distribution_guided_cache.model_e4_c_a0_c1_gpa_global_logits import run_test_tda as run_hierarchical_cache
...
def rebuild_combined_summary(summary_file, summary_original_file, summary_with_gpa_global_file):
    with summary_file.open("w", newline="") as out:
        writer = csv.writer(out)
        wrote_header = False

        for path in (summary_original_file, summary_with_gpa_global_file):
            if not path.exists():
                continue
...
def _build_weighted_fusion_prompt_embeddings(args, clip_model, classname, template):
    ...
    static_embeddings = _encode_prompt_embeddings(args, clip_model, static_texts) * static_weight
    dynamic_embeddings = _encode_prompt_embeddings(args, clip_model, dynamic_texts) * dynamic_weight
...
def build_text_distribution_once(args, clip_model, clip_weights_state, classnames, template):
    ...
    for class_index, classname in enumerate(classnames):
        if _is_weighted_prompt_fusion(template):
            class_embeddings = _build_weighted_fusion_prompt_embeddings(args, clip_model, classname, template)
        else:
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                text_dist = build_text_distribution_once(args, clip_model, clip_weights_state, classnames, template)
                print(f"[E4-C-A0-c1] text_dist classes: {len(text_dist)}")
                ...
                if args.wandb:
                    if isinstance(acc, dict):
                        wandb.log({
                            f"{args.dataset}/original": float(acc["original"]),
                            f"{args.dataset}/with_gpa_global": float(acc["with_gpa_global"]),
                        })
                    else:
                ...
                if isinstance(acc, dict):
                    print(
                        f"DONE: {cor_type}, "
                        f"original={float(acc['original']):.2f}, "
                        f"with_gpa_global={float(acc['with_gpa_global']):.2f}"
                    )
                else:
                    print(f"DONE: {cor_type}, acc={float(acc):.2f}")
                print("============================================================")

                return acc if isinstance(acc, dict) else float(acc)
...
def main():
    ...
    summary_original_file = run_dir / "summary_original.csv"
    summary_with_gpa_global_file = run_dir / "summary_with_gpa_global.csv"
    ...
    write_summary_header(summary_original_file)
    write_summary_header(summary_with_gpa_global_file)
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("E4-C-A0-c1 ULIP ModelNet-C severity=2 Z-score Accepted-History Text-Visual GPA Cache Dual Logits runner")
    ...
    for corruption in CORRUPTIONS:
        for severity in SEVERITIES:
            ...
            if not data_file.exists():
                ...
                append_summary_row(summary_original_file, missing_row)
                append_summary_row(summary_with_gpa_global_file, missing_row)
                rebuild_combined_summary(summary_file, summary_original_file, summary_with_gpa_global_file)
            ...
            try:
                ...
                if isinstance(acc, dict):
                    original_acc = float(acc["original"])
                    with_gpa_global_acc = float(acc["with_gpa_global"])
                else:
                    original_acc = float(acc)
                    with_gpa_global_acc = float(acc)

                append_summary_row(summary_original_file, [
                    exp_id,
                    args.dataset,
                    data_root,
                    corruption,
                ...
                append_summary_row(summary_with_gpa_global_file, [
                    exp_id,
                    args.dataset,
                    data_root,
                    corruption,
                    ...
                    method,
                    method_full,
                    "with_gpa_global_logits",
                    f"{with_gpa_global_acc:.2f}",
                    "done",
                    physical_gpu,
                    str(log_file),
                ])
                ...
                failed_gpa_global_row = list(failed_original_row)
                failed_gpa_global_row[11] = "with_gpa_global_logits"
                append_summary_row(summary_original_file, failed_original_row)
                append_summary_row(summary_with_gpa_global_file, failed_gpa_global_row)
                rebuild_combined_summary(summary_file, summary_original_file, summary_with_gpa_global_file)
                raise
    ...
    print(f"summary_original: {summary_original_file}")
    print(f"summary_with_gpa_global: {summary_with_gpa_global_file}")
    print("============================================================")
