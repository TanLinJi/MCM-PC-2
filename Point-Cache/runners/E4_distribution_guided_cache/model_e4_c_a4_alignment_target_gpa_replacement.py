"""
E4-C-A4: Alignment-Target GPA Replacement for Point-Cache.

核心思想：
1. 保留原始 Point-Cache 的 Global Entropy Cache，仍用于 global cache logits；
2. 保留 Global Prototype-Alignment Cache，简称 GPA Cache；
3. GPA Cache 未满时直接加入，用于 bootstrap；
4. GPA Cache 满后不再替换最高熵样本，而是替换 text-visual joint score 最低的样本；
5. 新样本必须通过低熵安全门控：entropy_new < max_entropy_in_GPA_cache；
6. 新样本必须改善 alignment 最差位置：joint_score_new > joint_score_target；
7. 只有进入 GPA Cache 的样本，其 patch_centers 才写入 local cache；
8. 最终预测公式保持 02_9_2 的 Point-Cache voting 不变。
"""
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual class-wise distribution score with alignment-target GPA replacement"
GPA_VARIANT_NAME = "E4-C-A4-alignment-target-gpa-replacement"
...
def _update_gpa_cache(
    ...
    """
    E4-C-A4：Alignment-target GPA-Cache 更新。

    未满时直接加入 GPA-Cache；满后选择当前类别 GPA-cache 中
    text-visual joint score 最低的样本作为替换目标。新样本需要满足：

        1. entropy_new < max_entropy_in_GPA_cache[pred]
        2. joint_score_new > joint_score_target

    这样 GPA-cache 维护目标从“替换最高熵样本 + joint veto”变为
    “修复 alignment 最差位置 + 低熵安全门控”。
    """
    ...
    def _cache_quality(entries):
        entropies = [_loss_value(item[1]) for item in entries]
        return {
            "max_entropy": max(entropies) if entropies else None,
            "mean_entropy": sum(entropies) / float(len(entropies)) if entropies else None,
        }

    def _cache_joint_summary(scored_entries):
        joint_scores = [entry["score"]["joint"] for entry in scored_entries]
        entropies = [entry["entropy"] for entry in scored_entries]
        return {
            "min_joint": min(joint_scores) if joint_scores else None,
            "mean_joint": sum(joint_scores) / float(len(joint_scores)) if joint_scores else None,
            "max_entropy": max(entropies) if entropies else None,
            "mean_entropy": sum(entropies) / float(len(entropies)) if entropies else None,
        }
    ...
    def record_event(
        ...
        target_entropy=None,
        target_index=None,
        cache_summary_before=None,
        cache_summary_after=None,
        target_entropy_rank=None,
        target_joint_rank=None,
    ):
        ...
        event_records.append({
            ...
            "decision": decision,
            "update_rule": "alignment_target_joint_min_with_cache_max_entropy_gate",
            ...
            "target_index_in_cache": None if target_index is None else int(target_index),
            "target_entropy": None if target_entropy is None else float(target_entropy),
            "target_joint_score": None if target_score is None else float(target_score["joint"]),
            "target_visual_score": None if target_score is None else float(target_score["visual"]),
            "target_text_score": None if target_score is None or target_score["text"] is None else float(target_score["text"]),
            "target_visual_score_for_joint": None if target_score is None else float(target_score["visual_for_joint"]),
            "target_text_score_for_joint": None if target_score is None or target_score["text_for_joint"] is None else float(target_score["text_for_joint"]),
            "target_visual_score_normalized": None if target_score is None else bool(target_score["visual_score_normalized"]),
            "target_text_score_normalized": None if target_score is None else bool(target_score["text_score_normalized"]),
            "cache_max_entropy": None if cache_summary_before is None else cache_summary_before["max_entropy"],
            "cache_min_joint_score": None if cache_summary_before is None else cache_summary_before["min_joint"],
            "cache_mean_entropy_before": None if cache_summary_before is None else cache_summary_before["mean_entropy"],
            "cache_mean_joint_score_before": None if cache_summary_before is None else cache_summary_before["mean_joint"],
            "cache_mean_entropy_after": None if cache_summary_after is None else cache_summary_after["mean_entropy"],
            "cache_mean_joint_score_after": None if cache_summary_after is None else cache_summary_after["mean_joint"],
            "new_minus_target_joint": (
                None if new_score is None or target_score is None else float(new_score["joint"] - target_score["joint"])
            ),
            "new_minus_cache_max_entropy": (
                None if cache_summary_before is None else float(cache_summary_before["max_entropy"] - curr_ent)
            ),
            "target_entropy_rank": None if target_entropy_rank is None else int(target_entropy_rank),
            "target_joint_rank": None if target_joint_rank is None else int(target_joint_rank),
            "text_gate_mode": TEXT_GATE_MODE,
    ...
    if len(gpa_cache[pred]) < shot_capacity:
        ...
        stats[f"{phase}_gpa_add_not_full_alignment_target"] += 1
        _update_visual_distribution(visual_dist, pred, global_item[0], stats, phase, "gpa_add")
        record_event(decision="add_not_full_alignment_target")
        return True

    curr_score = _joint_distribution_score(visual_dist, text_dist, pred, global_item[0], score_norm_state)
    if curr_score is None:
        stats[f"{phase}_gpa_reject_no_joint_score"] += 1
        record_event(decision="reject_no_joint_score", new_score=curr_score)
        return False

    scored_entries = []
    for index, item in enumerate(gpa_cache[pred]):
        score = _joint_distribution_score(visual_dist, text_dist, pred, item[0], score_norm_state)
        if score is None:
            stats[f"{phase}_gpa_reject_no_joint_score"] += 1
            record_event(decision="reject_no_joint_score", new_score=curr_score)
            return False
        scored_entries.append({
            ...
            "item": item,
            "score": score,
            "entropy": _loss_value(item[1]),
        })

    cache_summary_before = _cache_joint_summary(scored_entries)
    target_entry = min(scored_entries, key=lambda entry: entry["score"]["joint"])
    target_index = target_entry["index"]
    target_entropy = target_entry["entropy"]
    target_score = target_entry["score"]
    cache_max_entropy = cache_summary_before["max_entropy"]

    entropy_sorted = sorted(scored_entries, key=lambda entry: entry["entropy"], reverse=True)
    joint_sorted = sorted(scored_entries, key=lambda entry: entry["score"]["joint"])
    target_entropy_rank = next(
        rank for rank, entry in enumerate(entropy_sorted, start=1) if entry["index"] == target_index
    )
    target_joint_rank = next(
        rank for rank, entry in enumerate(joint_sorted, start=1) if entry["index"] == target_index
    )

    if curr_ent >= cache_max_entropy:
        stats[f"{phase}_gpa_reject_entropy"] += 1
        stats[f"{phase}_gpa_reject_entropy_not_below_cache_max"] += 1
        record_event(
            decision="reject_entropy_not_below_cache_max",
            new_score=curr_score,
            target_score=target_score,
            target_entropy=target_entropy,
            target_index=target_index,
            cache_summary_before=cache_summary_before,
            target_entropy_rank=target_entropy_rank,
            target_joint_rank=target_joint_rank,
        )
        return False

    norm_updates = _update_score_norm_state(score_norm_state, curr_score)
    norm_updates += _update_score_norm_state(score_norm_state, target_score)
    if norm_updates:
        stats[f"{phase}_score_norm_update"] += norm_updates
        stats[f"{phase}_score_norm_observed_alignment_target_pairs"] += 1

    if curr_score["joint"] > target_score["joint"]:
        gpa_cache[pred][target_index] = global_item
        gpa_local_cache[pred][target_index] = local_item
        _sort_gpa_pair_by_entropy(gpa_cache, gpa_local_cache, pred)

        scored_entries_after = []
        for item in gpa_cache[pred]:
            score = _joint_distribution_score(visual_dist, text_dist, pred, item[0], score_norm_state)
            if score is None:
                continue
            scored_entries_after.append({
                "score": score,
                "entropy": _loss_value(item[1]),
            })
        cache_summary_after = _cache_joint_summary(scored_entries_after)

        stats[f"{phase}_gpa_replace_accepted_history_text_visual_distribution"] += 1
        stats[f"{phase}_gpa_replace_alignment_target"] += 1
        _update_visual_distribution(visual_dist, pred, global_item[0], stats, phase, "gpa_replace")
        record_event(
            decision="replace_alignment_target",
            new_score=curr_score,
            target_score=target_score,
            target_entropy=target_entropy,
            target_index=target_index,
            cache_summary_before=cache_summary_before,
            cache_summary_after=cache_summary_after,
            target_entropy_rank=target_entropy_rank,
            target_joint_rank=target_joint_rank,
        )
        return True
    ...
    stats[f"{phase}_gpa_reject_joint_not_better_than_target"] += 1
    record_event(
        decision="reject_joint_not_better_than_target",
        new_score=curr_score,
        target_score=target_score,
        target_entropy=target_entropy,
        target_index=target_index,
...
def _save_gpa_stats(
    ...
    payload = {
        ...
        "e4_variant": "E4-C-A4-alignment-target-gpa-replacement",
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A4] Saved GPA replacement events to {event_path}")

    print(f"[E4-C-A4] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    print("*" * 10, "Building [global entropy] and [alignment-target GPA-controlled local] pos. cache ...", "*" * 10, "\n")
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # E4-C-A4：GPA 使用 alignment-target replacement 维护更干净的对齐缓存。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    print("[E4-C-A4] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A4] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A4] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A4] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A4] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A4] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A4] visual distribution classes:", len(visual_dist))
    print("[E4-C-A4] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A4] text gate mode:", TEXT_GATE_MODE)
    print("[E4-C-A4] text proto score scale:", TEXT_PROTO_SCORE_SCALE)
    print("[E4-C-A4] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A4] score norm state:", _summarize_score_norm_state(score_norm_state))
    ...
    # E4-C-A4 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E4-C-A4 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E4-C-A4 test accuracy: {:.2f}. ----\n".format(final_acc))
