"""
E4-C-A2: Diagnostic Dual-Low Trusted Fallback for Distribution-Guided GPA Cache.
...
7. A1 新增低熵 + 低能量可信样本回退：可信样本只需熵低于缓存最差样本即可替换；
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual class-wise distribution score with diagnostic dual-low trusted fallback"
GPA_VARIANT_NAME = "E4-C-A2-diagnostic-dual-low-trusted-fallback-accepted-history-text-visual-distribution-guided-gpa-cache"
...
def _update_gpa_cache(
    ...
    """
    E4-C-A1：带低熵 + 低能量可信回退的 GPA-Cache 更新。
    ...
    改动：
        若当前样本同时满足低熵和低能量，则更接近原始 Point-Cache 的熵替换逻辑；
        否则继续使用 accepted-history text-visual joint score。
    """
    ...
    def record_event(decision, old_entropy=None, new_score=None, old_score=None):
        ...
        event_records.append({
            ...
            "decision": decision,
            "update_rule": "dual_low_trusted_fallback_or_low_entropy_gate_accepted_history_text_visual_joint_score",
    ...
    if trusted_sample_info is not None and trusted_sample_info.get("is_trusted"):
        ...
        stats[f"{phase}_gpa_replace_trusted_low_entropy_low_energy"] += 1
        _update_visual_distribution(visual_dist, pred, global_item[0], stats, phase, "gpa_replace_trusted_low_entropy_low_energy")
        record_event(decision="replace_trusted_low_entropy_low_energy", old_entropy=worst_ent)
        _diag_record_sample(
            sample_diag_records,
            sample_context,
            pred,
            "replace_trusted_low_entropy_low_energy",
            True,
            curr_ent,
            worst_ent,
            trusted_sample_info,
    ...
    if curr_score["joint"] > worst_score["joint"]:
        ...
        _diag_record_sample(
            ...
            sample_context,
            pred,
            "replace_accepted_history_text_visual_distribution",
            True,
            curr_ent,
            worst_ent,
            trusted_sample_info,
...
def _save_gpa_stats(
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A2] Saved GPA replacement events to {event_path}")
    ...
    if sample_diag_records is not None and DIAG_SAVE_SAMPLE_ENERGY:
        ...
        print(f"[E4-C-A2] Saved sample energy/entropy diagnostics to {sample_path}")
    ...
    if replacement_diag_records is not None and DIAG_SAVE_REPLACEMENT_SNAPSHOT:
        ...
        with replacement_path.open("w", encoding="utf-8") as f:
            for record in replacement_diag_records:
                f.write(json.dumps(record, ensure_ascii=False) + "\n")
        print(f"[E4-C-A2] Saved GPA replacement diagnostics to {replacement_path}")

    print(f"[E4-C-A2] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for build_i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        # E4-C-A2：可信样本优先使用双低信号，同时记录熵/能量诊断。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    """
    E4-C-A2 diagnostic test-time adaptation.
    ...
    print("[E4-C-A2] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A2] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A2] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A2] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A2] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A2] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A2] visual distribution classes:", len(visual_dist))
    print("[E4-C-A2] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A2] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A2] score norm state:", _summarize_score_norm_state(score_norm_state))
    print("[E4-C-A2] trusted fallback mode:", TRUSTED_FALLBACK_MODE)
    print("[E4-C-A2] trusted entropy threshold:", TRUSTED_ENTROPY_THRESHOLD)
    print("[E4-C-A2] trusted energy z threshold:", TRUSTED_ENERGY_Z_THRESHOLD)
    print("[E4-C-A2] trusted energy min count:", TRUSTED_ENERGY_MIN_COUNT)
    print("[E4-C-A2] energy stats state:", _summarize_energy_stats_state(energy_stats_state))
    print("[E4-C-A2] save sample diagnostics:", DIAG_SAVE_SAMPLE_ENERGY)
    print("[E4-C-A2] save replacement diagnostics:", DIAG_SAVE_REPLACEMENT_SNAPSHOT)
    ...
    # E4-C-A2 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if i % args.print_freq == 0:
            print("---- E4-C-A2 test accuracy: {:.2f}. ----\n".format(sum(accuracies) / len(accuracies)))

    final_acc = sum(accuracies) / len(accuracies)
    print("---- ***Final*** E4-C-A2 test accuracy: {:.2f}. ----\n".format(final_acc))
