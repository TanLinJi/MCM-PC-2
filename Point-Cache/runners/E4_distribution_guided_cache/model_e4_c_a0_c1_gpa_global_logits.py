"""
E4-C-A0-c1: Z-score Accepted-History Text-Visual GPA Cache with Dual Logits.
...
8. 同一次测试同时输出原始最终 logits 公式，以及加入 GPA global cache logits 的增强公式。
"""
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual z-score distribution score with GPA global logits comparison"
GPA_VARIANT_NAME = "E4-C-A0-c1-zscore-accepted-history-text-visual-gpa-global-logits"
...
# ============================================================
# E4-C-A0-c1：Accepted-History Text-Visual 类别概率分布辅助函数
# ============================================================
...
def _joint_distribution_score(visual_dist, text_dist, pred, feat, score_norm_state=None):
    """
    计算 E4-C-A0-c1 的 text-visual joint score。
...
def _update_gpa_cache(
    ...
    """
    E4-C-A0-c1：Accepted-History Text-Visual 类别概率分布引导的 GPA-Cache 更新。
...
def _save_gpa_stats(
    ...
    payload = {
        ...
        "final_acc_with_gpa_global": None if not isinstance(acc, dict) else acc.get("with_gpa_global"),
        "stats": dict(stats),
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A0-c1] Saved GPA replacement events to {event_path}")

    print(f"[E4-C-A0-c1] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # E4-C-A0-c1：GPA 使用已被正缓存接受过的历史可信 visual_dist 评分。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    """
    E4-C-A0-c1 test-time adaptation.
    ...
    print("[E4-C-A0-c1] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A0-c1] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A0-c1] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A0-c1] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A0-c1] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A0-c1] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A0-c1] visual distribution classes:", len(visual_dist))
    print("[E4-C-A0-c1] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A0-c1] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A0-c1] score norm state:", _summarize_score_norm_state(score_norm_state))
    ...
    # E4-C-A0-c1 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    # accepted-history visual_dist。visual_dist 只累计曾被正缓存接受过的样本。
    accuracies_original = []
    accuracies_with_gpa_global = []
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        final_logits_with_gpa_global = final_logits_original.clone()

        if pos_enabled and gpa_cache:
            final_logits_with_gpa_global += compute_cache_logits(
                pc_feats,
                gpa_cache,
                pos_params["alpha"],
                pos_params["beta"],
        ...
        if neg_enabled and neg_cache:
            ...
            final_logits_with_gpa_global -= neg_logits

        acc_original = cls_acc(final_logits_original, target)
        acc_with_gpa_global = cls_acc(final_logits_with_gpa_global, target)
        accuracies_original.append(acc_original)
        accuracies_with_gpa_global.append(acc_with_gpa_global)
        wandb.log({
            "Averaged test accuracy/original": sum(accuracies_original) / len(accuracies_original),
            "Averaged test accuracy/with_gpa_global": sum(accuracies_with_gpa_global) / len(accuracies_with_gpa_global),
        }, commit=True)

        if i % args.print_freq == 0:
            print("---- E4-C-A0-c1 original test accuracy: {:.2f}. ----".format(
                sum(accuracies_original) / len(accuracies_original)
            ))
            print("---- E4-C-A0-c1 with GPA global test accuracy: {:.2f}. ----\n".format(
                sum(accuracies_with_gpa_global) / len(accuracies_with_gpa_global)
            ))

    final_acc_original = sum(accuracies_original) / len(accuracies_original)
    final_acc_with_gpa_global = sum(accuracies_with_gpa_global) / len(accuracies_with_gpa_global)
    final_acc = {
        "original": float(final_acc_original),
        "with_gpa_global": float(final_acc_with_gpa_global),
    }
    print("---- ***Final*** E4-C-A0-c1 original test accuracy: {:.2f}. ----".format(final_acc_original))
    print("---- ***Final*** E4-C-A0-c1 with GPA global test accuracy: {:.2f}. ----\n".format(final_acc_with_gpa_global))
