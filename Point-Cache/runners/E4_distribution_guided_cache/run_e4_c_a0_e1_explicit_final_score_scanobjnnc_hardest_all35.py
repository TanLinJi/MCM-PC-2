...
from runners.E4_distribution_guided_cache.model_e4_c_a0_e1_explicit_final_score import run_test_tda as run_hierarchical_cache
...
def _default_weight_name(alpha_g, alpha_l, alpha_n):
    return "ag{}_al{}_an{}".format(
        _float_token(alpha_g),
        _float_token(alpha_l),
        _float_token(alpha_n),
    )


def _parse_final_score_weights(raw):
    if raw is None or str(raw).strip() == "":
        return None
    ...
    for index, item in enumerate(str(raw).replace("\n", ";").split(";"), start=1):
        ...
        explicit_name = None
        if ":" in item:
            maybe_name, values = item.split(":", 1)
        ...
        if len(parts) != 3:
            raise ValueError(
                "Invalid E4_FINAL_SCORE_WEIGHTS item: "
                f"{item!r}. Expected alpha_g,alpha_l,alpha_n."
            )

        alpha_g, alpha_l, alpha_n = [float(part) for part in parts]
        name = explicit_name or _default_weight_name(alpha_g, alpha_l, alpha_n)
    ...
    if not weights:
        raise ValueError("E4_FINAL_SCORE_WEIGHTS was provided but no valid weights were parsed.")
...
def write_weight_summary(summary_file, rows_by_weight):
    cor_types = [
        f"{corruption}_{severity}"
        for corruption in CORRUPTIONS
        for severity in SEVERITIES
    ]
    out_file = summary_file.parent / "final_score_weight_summary.csv"
    with out_file.open("w", newline="") as f:
        ...
        writer.writerow([
            ...
            "alpha_n",
            "avg_all35",
            *[f"avg_s{severity}" for severity in SEVERITIES],
            *cor_types,
        ])
        for weight_name, payload in rows_by_weight.items():
            values = payload["acc_by_cor_type"]
            if not all(cor_type in values for cor_type in cor_types):
                continue
            avg = sum(values[cor_type] for cor_type in cor_types) / len(cor_types)
            severity_avgs = []
            for severity in SEVERITIES:
                severity_values = [
                    values[f"{corruption}_{severity}"]
                    for corruption in CORRUPTIONS
                ]
                severity_avgs.append(sum(severity_values) / len(severity_values))
            writer.writerow([
                weight_name,
                f"{payload['alpha_g']:g}",
                f"{payload['alpha_l']:g}",
                f"{payload['alpha_n']:g}",
                f"{avg:.4f}",
                *[f"{severity_avg:.4f}" for severity_avg in severity_avgs],
                *[f"{values[cor_type]:.2f}" for cor_type in cor_types],
            ])
...
def build_clip_weights_once(args, clip_model, clip_weights_state, classnames, template):
    if clip_weights_state["clip_weights"] is None:
        ...
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                "Classnames changed inside ScanObjNN-C all35 loop. "
                "clip_weights must be rebuilt, but this runner expects fixed ScanObjNN-C classes."
            )
...
def build_text_distribution_once(args, clip_model, clip_weights_state, classnames, template):
    if clip_weights_state.get("text_dist") is not None:
        if list(classnames) != clip_weights_state["classnames"]:
            raise RuntimeError(
                "Classnames changed inside ScanObjNN-C all35 loop. "
                "text_dist must be rebuilt, but this runner expects fixed ScanObjNN-C classes."
            )
...
def get_backbone_name(args):
    if args.lm3d == "ulip":
        if getattr(args, "ulip_version", "") == "ulip2":
            return "ULIP-2"
        return "ULIP"
    if args.lm3d == "openshape":
        return f"OpenShape-{getattr(args, 'oshape_version', 'vitg14')}"
    if args.lm3d == "uni3d":
        return "Uni3D"
    return args.lm3d
    ...
    with log_file.open("w") as lf:
        ...
        with redirect_stdout(Tee(sys.__stdout__, lf)), redirect_stderr(Tee(sys.__stderr__, lf)):
            ...
            try:
                ...
                print(f"data_file={Path(data_root) / args.sonn_variant / (cor_type + '.h5')}")
                ...
                if baseline_method == "zs":
                    ...
                    result = run_hierarchical_cache(args, cfg["positive"], cfg["negative"], test_loader, lm3d_model, clip_weights, text_dist=text_dist)
                else:
                    raise ValueError(f"Unsupported baseline_method: {baseline_method}")

                if baseline_method in ["zs", "zs_global"]:
                    result = {
                        "primary_acc": float(acc),
                        "primary_weight": {
                            "name": "single",
                            "alpha_g": "",
                            "alpha_l": "",
                ...
                print("============================================================")
                print(f"DONE: {cor_type}, primary_acc={float(result['primary_acc']):.2f}")
                print("============================================================")
...
def main():
    ...
    args.sonn_variant = os.environ.get("SONN_VARIANT", getattr(args, "sonn_variant", "hardest"))
    args.baseline_exp_id = exp_id
    args.baseline_result_root = str(result_root)
    args.baseline_method_full = method_full
    args.e4_variant = "09_2-E4-C-A0+E1-explicit-final-score"
    ...
    args.e4_final_score_weights = _parse_final_score_weights(os.environ.get("E4_FINAL_SCORE_WEIGHTS", ""))

    data_root = args.sonn_c_root
    backbone = get_backbone_name(args)
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("09_2 E4-C-A0+E1-textdist ScanObjNN-C all35 Explicit Final-Score runner")
        ...
        print(f"SONN variant: {args.sonn_variant}")
        print(f"Backbone: {backbone}")
        print(f"Result dir: {run_dir}")
        print("Variant: 09_2-E4-C-A0+E1-explicit-final-score")
        ...
        print("Explicit final-score formula: y = y_zs + alpha_g * y_g + alpha_l * y_l - alpha_n * y_n")
        print(f"E4_FINAL_SCORE_WEIGHTS: {os.environ.get('E4_FINAL_SCORE_WEIGHTS', '<config-default>')}")
        if args.e4_final_score_weights:
            for weight in args.e4_final_score_weights:
                print(
                    "  {name}: alpha_g={alpha_g}, alpha_l={alpha_l}, alpha_n={alpha_n}".format(
                        **weight
                    )
                )
        print(f"Model will be loaded once, then {len(CORRUPTIONS) * len(SEVERITIES)} ScanObjNN-C cor_type values will be evaluated.")
        print("============================================================")
        ...
        if method in ["zs_global", "zs_global_local"]:
            ...
            cfg["explicit_final_score_cache_capacity"] = {
            ...
            if args.e4_final_score_weights:
                cfg["explicit_final_score"] = {
                    "formula": "y = y_zs + alpha_g * y_g + alpha_l * y_l - alpha_n * y_n",
                    "weights": args.e4_final_score_weights,
                }
    ...
    for corruption in CORRUPTIONS:
        for severity in SEVERITIES:
            cor_type = f"{corruption}_{severity}"
            data_file = Path(data_root) / args.sonn_variant / f"{cor_type}.h5"
            ...
            try:
                ...
                for weight_result in result["weight_results"]:
                    weight_name = weight_result["name"]
                    alpha_g = weight_result["alpha_g"]
                    alpha_l = weight_result["alpha_l"]
                    alpha_n = weight_result["alpha_n"]
                    acc = float(weight_result["acc"])
                    ...
                    append_summary_row(summary_file, [
                        ...
                        method_full,
                        weight_name,
                        f"{alpha_g:g}" if isinstance(alpha_g, float) else alpha_g,
                        f"{alpha_l:g}" if isinstance(alpha_l, float) else alpha_l,
                        f"{alpha_n:g}" if isinstance(alpha_n, float) else alpha_n,
                        f"{acc:.2f}",
                        "done",
                        physical_gpu,
                        str(log_file),
                    ...
                    rows_by_weight.setdefault(weight_name, {
                        "alpha_g": alpha_g,
                        "alpha_l": alpha_l,
                        "alpha_n": alpha_n,
                        "acc_by_cor_type": {},
                    })
                    rows_by_weight[weight_name]["acc_by_cor_type"][cor_type] = acc
    ...
    print()
    weight_summary_file = write_weight_summary(summary_file, rows_by_weight)
    print("============================================================")
    print(f"All {len(CORRUPTIONS) * len(SEVERITIES)} ScanObjNN-C all35 runs finished: {exp_id}")
    print(f"summary: {summary_file}")
    print(f"final score weight summary: {weight_summary_file}")
    print("============================================================")
