"""
E4-02_12: Entropy Cache Capacity 5 + Protective Soft-Rejection GPA Cache.

核心思想：
1. 保留原始 Point-Cache 的 Global Entropy Cache，但容量从 3 扩大到 5；
2. 保留 Global Prototype-Alignment Cache（GPA Cache），容量保持 3；
...
6. GPA Cache 满后采用保护性软拒绝规则（同 02_11）；
7. 只有进入 GPA Cache 的样本，其 patch_centers 才写入 local cache；
8. 当前实验保持 02_9_2 的最终预测公式不变。

与 02_11 的区别：
    E4_ENTROPY_CACHE_CAPACITY=5 代替共享的 shot_capacity=3。
"""
...
GPA_VARIANT_NAME = "E4-02_12-entropy-cap5-protective-soft-rejection-gpa-cache"

# 独立控制全局熵缓存容量，GPA 缓存容量保持 shot_capacity（默认 3）
ENTROPY_CAPACITY = int(os.environ.get("E4_ENTROPY_CACHE_CAPACITY", "3"))


def _entropy_value(entropy):
    """Convert entropy tensor/scalar to python float for sorting and logging."""
    if torch.is_tensor(entropy):
        return float(entropy.detach().float().cpu().item())
...
def _sort_cache_by_entropy(cache, pred):
    cache[pred] = sorted(cache[pred], key=lambda x: _entropy_value(x[1]))


def _sort_local_cache_by_entropy(local_cache, pred):
    local_cache[pred] = sorted(local_cache[pred], key=lambda x: _entropy_value(x[1]))


def _sort_gpa_pair_by_entropy(gpa_cache, gpa_local_cache, pred):
    pairs = sorted(
        zip(gpa_cache[pred], gpa_local_cache[pred]),
        key=lambda pair: _entropy_value(pair[0][1]),
    )
...
def _update_entropy_cache(cache, pred, item, shot_capacity, stats, phase):
    ...
    if pred in cache:
        ...
        worst_ent = _entropy_value(cache[pred][-1][1])
        curr_ent = _entropy_value(item[1])
...
def _update_negative_cache(cache, pred, item, shot_capacity, stats, phase):
    ...
    item = [pc_feats, entropy, prob_map]
    """
    if pred in cache:
        if len(cache[pred]) < shot_capacity:
            cache[pred].append(item)
...
def _update_gpa_cache(
    ...
    """
    E4-02_11：保护性软拒绝（protective soft-rejection）GPA-Cache 更新。
    ...
    改动：
        满缓存时，联合分布分数不再必须高于被替换样本；
        只要它不低于当前类别 GPA cache 中最差的 joint score，就允许软接收。
    """
    if pred not in gpa_cache:
        gpa_cache[pred] = []
        gpa_local_cache[pred] = []

    curr_ent = _entropy_value(global_item[1])

    def record_event(decision, old_entropy=None, new_score=None, old_score=None, min_joint_score=None):
        ...
        event_records.append({
            ...
            "decision": decision,
            "update_rule": "low_entropy_gate_protective_soft_rejection_replace_highest_entropy",
            ...
            "min_joint_score": None if min_joint_score is None else float(min_joint_score),
            ...
            "new_minus_old": None if new_score is None or old_score is None else float(new_score["joint"] - old_score["joint"]),
            "new_minus_min": None if new_score is None or min_joint_score is None else float(new_score["joint"] - min_joint_score),
    ...
    worst_ent = _entropy_value(worst_global_item[1])
    ...
    existing_joint_scores = []
    for existing_item in gpa_cache[pred]:
        existing_score = _joint_distribution_score(visual_dist, text_dist, pred, existing_item[0], score_norm_state)
        if existing_score is None:
            ...
            return False
        existing_joint_scores.append(float(existing_score["joint"]))

    min_joint_score = min(existing_joint_scores) if existing_joint_scores else None
    if min_joint_score is None:
        stats[f"{phase}_gpa_reject_no_accepted_history_text_visual_distribution"] += 1
        record_event(
            decision="reject_no_accepted_history_text_visual_distribution",
            old_entropy=worst_ent,
    ...
    if curr_score["joint"] >= min_joint_score:
        gpa_cache[pred][-1] = global_item
        gpa_local_cache[pred][-1] = local_item
        ...
        stats[f"{phase}_gpa_replace_accepted_history_text_visual_distribution"] += 1
        stats[f"{phase}_gpa_replace_soft_accept"] += 1
        _update_visual_distribution(visual_dist, pred, global_item[0], stats, phase, "gpa_replace")
        record_event(
            decision="replace_protective_soft_accept",
            old_entropy=worst_ent,
            new_score=curr_score,
            old_score=worst_score,
            min_joint_score=min_joint_score,
    ...
    stats[f"{phase}_gpa_reject_below_min_joint"] += 1
    record_event(
        decision="reject_below_min_joint",
        old_entropy=worst_ent,
        new_score=curr_score,
        old_score=worst_score,
        min_joint_score=min_joint_score,
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    if include_prob_map:
        ...
        for pc, _, _, rgb in test_loader:
            ...
            pc_feats, patch_centers, clip_logits, entropy, prob_map, pred = get_logits(args, feature, lm3d_model, clip_weights)
    ...
    for pc, _, _, rgb in test_loader:
        ...
        global_item = [pc_feats, entropy]
        local_item = [patch_centers, entropy]
        ...
        entropy_accepted = _update_entropy_cache(entropy_cache, pred, global_item, ENTROPY_CAPACITY, stats, "build")
        if entropy_accepted:
        ...
        num_classes = clip_logits.size(1)
        full_num = ENTROPY_CAPACITY * num_classes
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        target, prop_entropy = target.cuda(), get_entropy(entropy, clip_weights)
        ...
        if pos_enabled:
            ...
            entropy_accepted = _update_entropy_cache(
                entropy_cache, pred, global_item, ENTROPY_CAPACITY, gpa_cache_stats, "test"
            )
            if entropy_accepted:
