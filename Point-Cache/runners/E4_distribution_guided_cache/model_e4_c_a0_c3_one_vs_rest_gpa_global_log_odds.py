"""
E4-C-A0-c3: Z-score Accepted-History Text-Visual GPA Cache with One-vs-Rest GPA Global Log-Odds.
...
8. 同一次测试同时输出原始最终 logits、raw GPA global logits、c2 prior-corrected evidence，
   以及 c3 one-vs-rest GPA global log-odds。
"""
...
CENTER_SOURCE_LABEL = "Accepted-history text-visual z-score distribution score with one-vs-rest GPA global log-odds comparison"
GPA_VARIANT_NAME = "E4-C-A0-c3-zscore-accepted-history-text-visual-one-vs-rest-gpa-global-log-odds"
...
# ============================================================
# E4-C-A0-c3：Accepted-History Text-Visual 类别概率分布辅助函数
# ============================================================
...
GPA_GLOBAL_PRIOR_GAMMAS_RAW = os.environ.get("E4_GPA_GLOBAL_PRIOR_GAMMAS", "0.1,0.25,0.5")
GPA_GLOBAL_ODDS_GAMMAS_RAW = os.environ.get(
    "E4_GPA_GLOBAL_ODDS_GAMMAS",
    os.environ.get("E4_GPA_GLOBAL_GAMMAS", os.environ.get("GPA_GLOBAL_GAMMAS", "0.05,0.1,0.25")),
)
...
if GPA_GLOBAL_PRIOR != "uniform":
    raise ValueError(f"Unsupported E4_GPA_GLOBAL_PRIOR: {GPA_GLOBAL_PRIOR}. Use 'uniform' for E4-C-A0-c3.")
...
def _parse_gamma_list(raw):
    ...
    for part in raw.split(","):
        ...
        value = float(text)
        if value < 0:
            raise ValueError(f"GPA global gammas must be non-negative, got {value}")
        gammas.append(value)
    if not gammas:
        raise ValueError("GPA global gamma list must contain at least one value")
    return tuple(gammas)


GPA_GLOBAL_PRIOR_GAMMAS = _parse_gamma_list(GPA_GLOBAL_PRIOR_GAMMAS_RAW)
GPA_GLOBAL_ODDS_GAMMAS = _parse_gamma_list(GPA_GLOBAL_ODDS_GAMMAS_RAW)
...
def _one_vs_rest_formula_label(gamma):
    return f"one_vs_rest_gpa_global_odds_gamma_{_gamma_label(gamma)}"


def get_formula_labels():
    return [ORIGINAL_FORMULA, RAW_GPA_GLOBAL_FORMULA] + [
        _prior_corrected_formula_label(gamma) for gamma in GPA_GLOBAL_PRIOR_GAMMAS
    ] + [
        _one_vs_rest_formula_label(gamma) for gamma in GPA_GLOBAL_ODDS_GAMMAS
    ]
...
def get_formula_summary_slug(formula):
    ...
    prefix = "one_vs_rest_gpa_global_odds_gamma_"
    if formula.startswith(prefix):
        return "one_vs_rest_odds_gamma_" + formula[len(prefix):]
...
def _joint_distribution_score(visual_dist, text_dist, pred, feat, score_norm_state=None):
    """
    计算 E4-C-A0-c3 的 text-visual joint score。
...
def _update_gpa_cache(
    ...
    """
    E4-C-A0-c3：Accepted-History Text-Visual 类别概率分布引导的 GPA-Cache 更新。
...
@torch.no_grad()
def compute_gpa_global_density(pc_feats, gpa_cache, beta, clip_weights):
    """
    Compute class-size-normalized GPA global density r_c(x).

    r_c(x) = mean_{g_i in G_c} exp(beta * (cos(x, g_i) - 1)).
    """
    num_classes = clip_weights.size(1)
    pc_feats_float = pc_feats.detach().float()
    density = torch.zeros((pc_feats.size(0), num_classes), device=pc_feats.device, dtype=torch.float32)
...
@torch.no_grad()
def compute_prior_corrected_gpa_global_evidence_from_density(density):
    """
    Compute c2 diagnostic evidence:
        e_G(c, x) = log(r_c(x) + eps) - log(m(x) + eps).

    m(x) is class-independent and therefore cancels under softmax/argmax, but
    this diagnostic is kept for c2-vs-c3 comparison in the same run.
    """
    num_classes = density.size(1)
...
@torch.no_grad()
def compute_one_vs_rest_gpa_global_log_odds_from_density(density):
    """
    Compute c3 evidence:
        e_odds(c, x) = log(r_c(x) + eps) - log(r_not_c(x) + eps)

    where:
        r_not_c(x) = (sum_j r_j(x) - r_c(x)) / (C - 1).

    The second term depends on c and is not removed by softmax/argmax.
    """
    num_classes = density.size(1)
    if num_classes <= 1:
        return torch.zeros_like(density), {
            "rest_density_min": None,
            "rest_density_max": None,
        }

    rest_density = (density.sum(dim=1, keepdim=True) - density) / float(num_classes - 1)
    evidence = torch.log(density.clamp_min(GPA_GLOBAL_EPS)) - torch.log(rest_density.clamp_min(GPA_GLOBAL_EPS))
    stats = {
        "rest_density_min": float(rest_density.detach().float().min().cpu().item()),
        "rest_density_max": float(rest_density.detach().float().max().cpu().item()),
    }
    return evidence, stats
...
def _save_gpa_stats(
    ...
    payload = {
        ...
        "gpa_global_evidence_mode": "one_vs_rest_log_odds",
        "gpa_global_diagnostic_modes": ["raw_gpa_global_logits", "prior_corrected_log_likelihood_ratio"],
        ...
        "gpa_global_prior_gammas": [float(gamma) for gamma in GPA_GLOBAL_PRIOR_GAMMAS],
        "gpa_global_odds_gammas": [float(gamma) for gamma in GPA_GLOBAL_ODDS_GAMMAS],
        ...
        "final_acc_prior_corrected": {
            ...
            for gamma in GPA_GLOBAL_PRIOR_GAMMAS
        },
        "final_acc_one_vs_rest_odds": {
            _gamma_label(gamma): None if not isinstance(acc, dict) else acc.get(_one_vs_rest_formula_label(gamma))
            for gamma in GPA_GLOBAL_ODDS_GAMMAS
        },
        "stats": dict(stats),
    ...
    if event_records is not None:
        ...
        print(f"[E4-C-A0-c3] Saved GPA replacement events to {event_path}")

    print(f"[E4-C-A0-c3] Saved GPA stats to {out_dir / filename}")
...
def build_cache_in_advance(args, test_loader, lm3d_model, clip_weights, shot_capacity, include_prob_map=False, text_dist=None):
    ...
    for pc, _, _, rgb in test_loader:
        ...
        # E4-C-A0-c3：GPA 使用已被正缓存接受过的历史可信 visual_dist 评分。
        _update_gpa_cache(
            gpa_cache,
            gpa_local_cache,
            visual_dist,
...
def run_test_tda(args, pos_cfg, neg_cfg, test_loader, lm3d_model, clip_weights, text_dist=None):
    """
    E4-C-A0-c3 test-time adaptation.
    ...
    print("[E4-C-A0-c3] len(entropy_cache):", len(entropy_cache))
    print("[E4-C-A0-c3] len(gpa_cache):", len(gpa_cache))
    print("[E4-C-A0-c3] len(gpa_local_cache):", len(gpa_local_cache))
    print("[E4-C-A0-c3] entropy cache total:", sum(len(v) for v in entropy_cache.values()))
    print("[E4-C-A0-c3] gpa cache total:", sum(len(v) for v in gpa_cache.values()))
    print("[E4-C-A0-c3] gpa local cache total:", sum(len(v) for v in gpa_local_cache.values()))
    print("[E4-C-A0-c3] visual distribution classes:", len(visual_dist))
    print("[E4-C-A0-c3] text distribution classes:", 0 if text_dist is None else len(text_dist))
    print("[E4-C-A0-c3] score norm mode:", SCORE_NORM_MODE)
    print("[E4-C-A0-c3] score norm state:", _summarize_score_norm_state(score_norm_state))
    print("[E4-C-A0-c3] GPA global prior:", GPA_GLOBAL_PRIOR)
    print("[E4-C-A0-c3] GPA global eps:", GPA_GLOBAL_EPS)
    print("[E4-C-A0-c3] GPA global beta override:", GPA_GLOBAL_BETA_OVERRIDE or "positive.beta")
    print("[E4-C-A0-c3] GPA global prior-corrected diagnostic gammas:", ", ".join(_gamma_label(gamma) for gamma in GPA_GLOBAL_PRIOR_GAMMAS))
    print("[E4-C-A0-c3] GPA global one-vs-rest odds gammas:", ", ".join(_gamma_label(gamma) for gamma in GPA_GLOBAL_ODDS_GAMMAS))
    ...
    # E4-C-A0-c3 保留 build 阶段形成的 EntropyCache、GPA-Cache、GPA-local-cache 和
    ...
    for i, (pc, target, _, rgb) in enumerate(test_loader):
        ...
        if pos_enabled and gpa_cache:
            ...
            gpa_density, gpa_density_stats = compute_gpa_global_density(
                pc_feats,
                gpa_cache,
                gpa_global_beta,
                clip_weights,
            )
            gpa_prior_evidence, gpa_prior_stats = compute_prior_corrected_gpa_global_evidence_from_density(gpa_density)
            gpa_odds_evidence, gpa_odds_stats = compute_one_vs_rest_gpa_global_log_odds_from_density(gpa_density)

            gpa_cache_stats["test_gpa_global_evidence_eval"] += 1
            gpa_cache_stats["test_gpa_global_evidence_empty_classes_total"] += int(gpa_density_stats["empty_classes"])
            gpa_cache_stats["test_gpa_global_evidence_nonempty_classes_total"] += int(gpa_density_stats["nonempty_classes"])
            if gpa_prior_stats["normalizer_min"] is not None:
                gpa_cache_stats["test_gpa_global_prior_normalizer_observed"] += 1
            if gpa_odds_stats["rest_density_min"] is not None:
                gpa_cache_stats["test_gpa_global_odds_rest_density_observed"] += 1

            for gamma in GPA_GLOBAL_PRIOR_GAMMAS:
                final_logits_by_formula[_prior_corrected_formula_label(gamma)] = (
                    final_logits_original + float(gamma) * gpa_prior_evidence
                )
            for gamma in GPA_GLOBAL_ODDS_GAMMAS:
                final_logits_by_formula[_one_vs_rest_formula_label(gamma)] = (
                    final_logits_original + float(gamma) * gpa_odds_evidence
                )
        ...
        if i % args.print_freq == 0:
            print("---- E4-C-A0-c3 original test accuracy: {:.2f}. ----".format(
                sum(accuracies[ORIGINAL_FORMULA]) / len(accuracies[ORIGINAL_FORMULA])
            ))
            print("---- E4-C-A0-c3 raw GPA global test accuracy: {:.2f}. ----".format(
                sum(accuracies[RAW_GPA_GLOBAL_FORMULA]) / len(accuracies[RAW_GPA_GLOBAL_FORMULA])
            ))
            for gamma in GPA_GLOBAL_PRIOR_GAMMAS:
                formula = _prior_corrected_formula_label(gamma)
                print("---- E4-C-A0-c3 prior-corrected gamma={} test accuracy: {:.2f}. ----".format(
                    _gamma_label(gamma),
                    sum(accuracies[formula]) / len(accuracies[formula])
                ))
            for gamma in GPA_GLOBAL_ODDS_GAMMAS:
                formula = _one_vs_rest_formula_label(gamma)
                print("---- E4-C-A0-c3 one-vs-rest odds gamma={} test accuracy: {:.2f}. ----".format(
                    _gamma_label(gamma),
                    sum(accuracies[formula]) / len(accuracies[formula])
                ))
            print()
    ...
    print("---- ***Final*** E4-C-A0-c3 original test accuracy: {:.2f}. ----".format(final_acc[ORIGINAL_FORMULA]))
    print("---- ***Final*** E4-C-A0-c3 raw GPA global test accuracy: {:.2f}. ----".format(final_acc[RAW_GPA_GLOBAL_FORMULA]))
    for gamma in GPA_GLOBAL_PRIOR_GAMMAS:
        formula = _prior_corrected_formula_label(gamma)
        print("---- ***Final*** E4-C-A0-c3 prior-corrected gamma={} test accuracy: {:.2f}. ----".format(
            _gamma_label(gamma),
            final_acc[formula],
        ))
    for gamma in GPA_GLOBAL_ODDS_GAMMAS:
        formula = _one_vs_rest_formula_label(gamma)
        print("---- ***Final*** E4-C-A0-c3 one-vs-rest odds gamma={} test accuracy: {:.2f}. ----".format(
            _gamma_label(gamma),
            final_acc[formula],
        ))
    print()
