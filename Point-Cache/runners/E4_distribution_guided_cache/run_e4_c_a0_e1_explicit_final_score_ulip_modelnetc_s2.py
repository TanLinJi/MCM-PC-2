...
def write_weight_summary(summary_file, rows_by_weight):
    ...
    with out_file.open("w", newline="") as f:
        ...
        for weight_name, payload in rows_by_weight.items():
            values = payload["acc_by_corruption"]
            if not all(corruption in values for corruption in corruptions):
                continue
            avg = sum(values[corruption] for corruption in corruptions) / len(corruptions)
            writer.writerow([
                weight_name,
                f"{payload['alpha_g']:g}",
                f"{payload['alpha_l']:g}",
                f"{payload['alpha_n']:g}",
                f"{avg:.4f}",
                *[f"{values[corruption]:.2f}" for corruption in corruptions],
            ])
...
def main():
    ...
    with redirect_stdout(Tee(sys.__stdout__, init_log_buffer)), redirect_stderr(Tee(sys.__stderr__, init_log_buffer)):
        print("============================================================")
        print("09_2 E4-C-A0+E1-textdist ULIP ModelNet-C severity=2 Explicit Final-Score runner")
        ...
        for severity in SEVERITIES:
            ...
            try:
                ...
                for weight_result in result["weight_results"]:
                    ...
                    rows_by_weight[weight_name]["acc_by_corruption"][corruption] = acc
