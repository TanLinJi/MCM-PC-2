...
# E1: point-cloud-aware manual prompt subset.
#
# manual_3d_prompts is automatically filtered from the original
# Point-Cache text_prompts. It keeps templates that are more related
# to point clouds, 3D objects, shapes, geometry, models, or scenes,
# and removes templates that are clearly 2D image-style prompts.
...
def _is_point_cloud_aware_prompt(template):
    lower_template = template.lower()

    if any(keyword in lower_template for keyword in _IMAGE_STYLE_PROMPT_KEYWORDS):
        return False

    return any(keyword in lower_template for keyword in _POINT_CLOUD_PROMPT_KEYWORDS)


manual_3d_prompts = [
    template for template in text_prompts
    if _is_point_cloud_aware_prompt(template)
]

if len(manual_3d_prompts) == 0:
    raise RuntimeError(
        "manual_3d_prompts is empty. Please check the prompt filtering rules in datasets/templates.py."
    )
