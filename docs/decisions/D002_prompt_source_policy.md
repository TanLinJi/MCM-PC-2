...
The goal is to study whether text prototypes can be improved by using point-cloud-aware templates and LLM-generated class-level descriptions.
...
`llm_static` means a static prompt source generated by LLM and saved as JSON before experiments.
...
`llm_dynamic_init` means LLM prompts generated at the beginning of each experiment run.

中文理解：`llm_dynamic_init` 指在每次实验启动时，根据当前数据集的候选类别名称调用 LLM 生成文本描述。生成后保存并冻结，整个测试流中不再更新。
...
`manual3d_llm_dynamic_init` means a mixed prompt source combining `manual_3d` and `llm_dynamic_init`.
...
2. call LLM to generate class-level point-cloud-aware descriptions;
...
LLM API key must never be committed to the repository.
...
1. read from `Point-Cache/llm/secrets/llm_api_key.txt` if the file exists;
2. otherwise read from the environment variable `LLM_API_KEY`;
